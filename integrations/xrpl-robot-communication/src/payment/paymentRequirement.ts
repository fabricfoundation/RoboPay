...
export function createPaymentRequirement(input: {
  ...
  const extra: PaymentRequirement["extra"] = {
    ...
    expiresAt,
    invoiceId,
    sourceTag: input.config.sourceTag
  };
  if (input.config.issuer) extra.issuer = input.config.issuer;
  if (input.config.destinationTag !== undefined) extra.destinationTag = input.config.destinationTag;
...
function createInvoiceId(input: Record<string, unknown>): string {
  return `xrpl-invoice-${createHash("sha256").update(stableStringify(input)).digest("hex")}`;
}
