...
import { createMockPaymentPayload, type MockPaymentScenario } from "../src/payment/xrplX402.js";
...
export async function startTestStack(): Promise<TestStack> {
  const facilitator = await listen(createMockXrplFacilitatorApp());
...
export function paymentHeaderFromRequirement(requirement: PaymentRequirement, mutate: (requirement: PaymentRequirement) => void) {
  const cloned = JSON.parse(JSON.stringify(requirement)) as PaymentRequirement;
  mutate(cloned);
  return paymentHeader(cloned);
}
