...
// action boundary remains closed when a reachable facilitator returns either a
// nil response or HTTP-successful isValid:false verdict. The handler must
// never run and settlement must never be attempted.
func TestDeferredSettlementGateRejectsInvalidFacilitatorVerification(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil verification response", verification: nil},
		{
