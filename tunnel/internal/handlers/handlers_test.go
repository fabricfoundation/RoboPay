...
func TestRobotAndSkillDiscovery(t *testing.T) {
	_, _, router := newTestHandlers(t, "test-discovery-robot")
	...
	if payload.RobotID != "test-discovery-robot" || len(payload.Skills) != 2 {
...
func TestPostAction_ImmediateAcceptedPendingContract(t *testing.T) {
	_, publisher, router := newTestHandlers(t, "test-sim-robot")

	res := postAction(router, `{"action":"navigate_obstacle_course","robot_id":"test-sim-robot","action_id":"action-123","idempotency_key":"action-123","params":{"target_object":"apple"}}`, nil)
	...
	if event["robot_id"] != "test-sim-robot" {
