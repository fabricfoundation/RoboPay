...
func TestRobotAndSkillDiscovery(t *testing.T) {
	_, _, router := newTestHandlers(t, "generic-discovery-test")
	...
	if payload.RobotID != "generic-discovery-test" || len(payload.Skills) != 2 {
...
func TestPostAction_ImmediateAcceptedPendingContract(t *testing.T) {
	_, publisher, router := newTestHandlers(t, "generic-sim-01")

	res := postAction(router, `{"action":"navigate_obstacle_course","robot_id":"generic-sim-01","action_id":"action-123","idempotency_key":"action-123","params":{"target_object":"apple"}}`, nil)
	...
	if event["robot_id"] != "generic-sim-01" {
