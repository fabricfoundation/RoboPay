"""End-to-end demo client for door-arm-001 (criterion #1, #10).
...
Usage
    ...
    python -m flow.demo --door stuck        # a real failure -> no settlement
    python -m flow.demo --door out_of_range # unreachable scene
    python -m flow.demo --all               # all three scenes, summary table
...
SCENES = ["open", "stuck", "out_of_range"]
...
def run_once(relay: Relay, executor_probe, door: str, verbose: bool = True,
             payment_mode: str = "demo") -> dict:
    key = f"demo-{door}-{int(time.time() * 1000)}"
    request = {"robotId": ROBOT_ID, "skill": "open_door",
               "params": {"door": door}, "idempotencyKey": key}

    if verbose:
        step(2, f"request_action  params={{'door': '{door}'}}  (no payment attached)")
    ...
    if payment_mode == "x402":
        receipt = X402Receipt.for_scene(accepts, door, 1).to_dict()
        if verbose:
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="door-arm-001 paid-flow demo")
    ap.add_argument("--door", default="open", help=f"one of {SCENES}")
    ...
    print("=" * 68)
    print(f" RoboPay Tier 1 demo -- {ROBOT_ID} / open_door")
    ...
    if args.all:
        ...
        for door in SCENES:
            ...
            print(f" scene: {door}")
            print("-" * 68)
            res = run_once(relay, executor, door, verbose=False,
                           payment_mode=args.payment_mode)
            m = res.get("metrics") or {}
            rows.append((door, res.get("status"), res.get("message"),
                         res.get("settled"), m.get("doorAngle", 0.0),
                         m.get("contactForce", 0.0), m.get("stepsUsed", 0),
                         m.get("handleState", "-")))
            print(f" status={res.get('status')}  reason={res.get('message')}  "
                  f"settled={res.get('settled')}")
            print(f" handleState={m.get('handleState')}  angle={m.get('doorAngle'):.3f} rad  "
            ...
            if node:
                node.stop()
        print("\n" + "=" * 78)
        print(f" {'scene':<13}{'status':<11}{'reason':<13}{'angle(rad)':>10}"
              f"{'force(N)':>10}{'steps':>7}{'settled':>9}")
        print("-" * 78)
        for door, status, message, settled, angle, force, steps, _state in rows:
            print(f" {door:<13}{status:<11}{str(message):<13}{angle:>10.3f}"
    ...
    relay, executor, node = build_relay(args.engine, args.transport)
    result = run_once(relay, executor, args.door,
                      payment_mode=args.payment_mode)
    if node:
        node.stop()
    print("\n" + "=" * 68)
