"""Exercise tron1-001's x402 payment gate through the real Go Tunnel binary.
...
# bridge/tron1-001/tests -> bridge/tron1-001 -> repo root
PACKAGE_ROOT = Path(__file__).resolve().parents[1]
ROOT = PACKAGE_ROOT.parents[1]
SKILL_CATALOG = (
    ROOT
    / "registry/vendors/laok/tron1-001-arm-001"
    / "laok.tron1-001-arm-001.loco.v1/skill-catalog.json"
)
BRIDGE_PYTHONPATH = str(PACKAGE_ROOT)
ROBOT_ID = "tron1_001_payment_gate"
ZENOH_TEST_PORT = int(os.environ.get("UNITREE_TRON1_PAYMENT_GATE_ZENOH_PORT", "7447"))
PRICE = "0.10"
...
class UnitreeTRON1PaymentGateTests(unittest.TestCase):
    ...
    def test_unpaid_malformed_and_facilitator_rejected_requests_fail_closed(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="tron1_001_payment_gate_") as temp_dir:
                ...
                tampered_id = f"tron1-tampered-{uuid.uuid4().hex}"
                ...
                print("[UNITREE_TRON1 DISCOVERY] robot + skills + price: OK")
                print("[UNITREE_TRON1 PAYMENT GATE] unpaid/malformed/isValid:false -> HTTP 402, zero ActionEvents")
        finally:
    ...
    def test_paid_action_publishes_and_settles(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="tron1_001_paid_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                paid_id = f"tron1-paid-{uuid.uuid4().hex}"
                ...
                print("[UNITREE_TRON1 PAID] verified payment -> ActionEvent -> correlated result -> settle: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_failed_execution_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="tron1_001_fail_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                failed_id = f"tron1-fail-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[UNITREE_TRON1 FAILURE] failed execution -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_timeout_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="tron1_001_timeout_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                timeout_id = f"tron1-timeout-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[UNITREE_TRON1 TIMEOUT] no simulator result -> timeout -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
