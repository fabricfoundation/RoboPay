"""tron1-001 --- PyBullet backend (sim-to-sim cross-check).
...
stage step counts, gait constants, scene layout -- is imported from tron1_spec.py,
...
from tron1_spec import (
    LEG_JOINTS, HIP_MIN, HIP_MAX, KNEE_MIN, KNEE_MAX,
    STAND_Z, TORSO_H, TORSO_L, HIP_X_OFFSET, THIGH_LEN, SHANK_LEN, FOOT_H, FOOT_HALF,
    STEP_LEN, STEP_CLEAR, SWING_STEPS, TIMESTEP, WALK_VEL, OBSTACLE_HALF_X,
    OBSTACLE_CLEAR_Z, resolve_scene, leg_ik, build_metrics, WalkResult,
    DEFAULT_BUDGET,
)
...
def _robot_urdf() -> str:
    ...
    Joint order is fixed: torso_x (prismatic along X) then the eight leg
    hinges (fl/fr/rl/rr, hip then knee each), so the static sim2sim test can
    assert the URDF matches the spec.
    """
    link_blocks = []
    for leg in ("fl", "fr", "rl", "rr"):
        link_blocks.append(
            ...
            "    " + _inertial(1.0) + "\n"
            "    <visual><origin xyz=\"0 0 {THIGH_HALF:.3f}\"/><geometry><capsule radius=\"0.028\" length=\"{THIGH_LEN:.3f}\"/></geometry></visual>\n"
            "    <collision><origin xyz=\"0 0 {THIGH_HALF:.3f}\"/><geometry><capsule radius=\"0.028\" length=\"{THIGH_LEN:.3f}\"/></geometry></collision>\n"
            "  </link>\n"
            "  <link name=\"" + leg + "_shank\">\n"
            "    " + _inertial(0.8) + "\n"
            "    <visual><origin xyz=\"0 0 {SHANK_HALF:.3f}\"/><geometry><capsule radius=\"0.024\" length=\"{SHANK_LEN:.3f}\"/></geometry></visual>\n"
            "    <collision><origin xyz=\"0 0 {SHANK_HALF:.3f}\"/><geometry><capsule radius=\"0.024\" length=\"{SHANK_LEN:.3f}\"/></geometry></collision>\n"
            "    <visual><origin xyz=\"0 0 {SHANK_FOOT_HALF:.3f}\"/><geometry><box size=\"{FOOT_HALF:.3f} 0.06 {FOOT_H:.3f}\"/></geometry></visual>\n"
            "    <collision><origin xyz=\"0 0 {SHANK_FOOT_HALF:.3f}\"/><geometry><box size=\"{FOOT_HALF:.3f} 0.06 {FOOT_H:.3f}\"/></geometry></collision>\n"
            "  </link>\n"
        )
        link_blocks.append(
            "  <joint name=\"" + leg + "_hip\" type=\"revolute\" parent=\"torso\" child=\"" + leg + "_thigh\">\n"
            "    <origin xyz=\"0 0 {TORSO_NEG_HALF:.3f}\"/>\n"
            "    <axis xyz=\"0 1 0\"/>\n"
            "    <limit lower=\"{HIP_MIN}\" upper=\"{HIP_MAX}\" effort=\"200\" velocity=\"10\"/>\n"
            "  </joint>\n"
            "  <joint name=\"" + leg + "_knee\" type=\"revolute\" parent=\"" + leg + "_thigh\" child=\"" + leg + "_shank\">\n"
            "    <origin xyz=\"0 0 {THIGH_NEG:.3f}\"/>\n"
            "    <axis xyz=\"0 1 0\"/>\n"
            "    <limit lower=\"{KNEE_MIN}\" upper=\"{KNEE_MAX}\" effort=\"200\" velocity=\"10\"/>\n"
            "  </joint>\n"
        )
    links = "".join(link_blocks)
    return (
        "<?xml version=\"1.0\"?>\n"
        "<robot name=\"tron1-001\">\n"
        "  <link name=\"base\">\n"
        "    " + _inertial(0.0) + "\n"
        ...
        "  <link name=\"torso\">\n"
        "    " + _inertial(5.0) + "\n"
        "    <visual><origin xyz=\"0 0 0\"/><geometry><box size=\"{TORSO_L:.3f} 0.18 {TORSO_H:.3f}\"/></geometry>\n"
        "      <material name=\"torso_m\"><color rgba=\"0.2 0.5 0.9 1\"/></material></visual>\n"
        "    <collision><origin xyz=\"0 0 0\"/><geometry><box size=\"{TORSO_L:.3f} 0.18 {TORSO_H:.3f}\"/></geometry></collision>\n"
        "  </link>\n"
        ...
        + links +
        "</robot>\n"
    ).format(TORSO_L=TORSO_L, TORSO_H=TORSO_H, TORSO_HALF=TORSO_H/2.0, TORSO_NEG_HALF=-TORSO_H/2.0, THIGH_NEG=-THIGH_LEN,
             THIGH_LEN=THIGH_LEN, THIGH_HALF=THIGH_LEN/2.0,
             SHANK_LEN=SHANK_LEN, SHANK_HALF=SHANK_LEN/2.0,
             SHANK_FOOT_HALF=SHANK_LEN + FOOT_H/2.0,
             FOOT_HALF=FOOT_HALF, FOOT_H=FOOT_H,
             STAND_Z=STAND_Z, HIP_MIN=HIP_MIN, HIP_MAX=HIP_MAX,
...
class PyBulletSimulator:
    ...
    def _drive(self, virtual_x: float):
        ...
        for leg in ("fl", "fr", "rl", "rr"):
            hx = virtual_x + (HIP_X_OFFSET if leg in ("fl", "fr")
                              else -HIP_X_OFFSET)
            hz = STAND_Z - TORSO_H / 2.0
            hip_a, knee_a = leg_ik(hx - hx, tz - hz)
    ...
    def _foot_targets(self, step: int, obstacles, advancing: bool):
        """Trot gait: diagonal pairs (fl+rr) / (fr+rl) swing alternately."""
        if not advancing:
            g = _ground_z(self._virtual_x, obstacles) + FOOT_H
            return {leg: (self._virtual_x
                          + (HIP_X_OFFSET if leg in ("fl", "fr")
                             else -HIP_X_OFFSET), g)
                    for leg in ("fl", "fr", "rl", "rr")}
        half = SWING_STEPS
        stride_no = step // half
        t = (step % half) / half
        pair_a_swings = (stride_no % 2 == 0)     # fl+rr on even strides
        targets = {}
        for leg in ("fl", "fr", "rl", "rr"):
            swing = ((leg in ("fl", "rr") and pair_a_swings) or
                     (leg in ("fr", "rl") and not pair_a_swings))
            hx = self._virtual_x + (HIP_X_OFFSET if leg in ("fl", "fr")
                                    else -HIP_X_OFFSET)
            if not swing:
                targets[leg] = (hx, _ground_z(hx, obstacles) + FOOT_H)
            else:
                ...
                swing_z = (_ground_z(swing_x, obstacles) + FOOT_H
                           + max(STEP_CLEAR, OBSTACLE_CLEAR_Z)
                           * math.sin(math.pi * t))
