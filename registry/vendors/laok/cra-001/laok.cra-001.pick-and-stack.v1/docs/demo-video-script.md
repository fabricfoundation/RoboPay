# Demo Video Script — cra-001 / `pick_and_stack`
...
- **On screen:** `README.md` header, then:
  ...
  cra-001  ·  skill: pick_and_stack  ·  engine: MuJoCo 3.11
  ```
- **Voiceover:** "This is cra-001, a paid robotic-arm skill running inside
...
- **On screen:** `tree -L 2` (or `ls`), then `cat profiles/skills.yaml` (just the
  `pick_and_stack` pricing + `settlement: on-success-only` block) and
...
- **On screen:** run `python -m flow.demo --object cube`, let it print the 10 steps:
  1. `list_skills` (free) → sees `pick_and_stack: 0.10 USDC`
...
- **Voiceover:** "Fork, drop `bridge/cra-001/` into RoboPay, push, and the
