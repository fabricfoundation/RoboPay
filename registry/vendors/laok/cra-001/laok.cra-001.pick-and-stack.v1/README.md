# cra-001 — RoboPay Tier 1 bridge (Simulator Skill Execution)

A paid `pick_and_stack` skill executed by **real physics**, driven over **Zenoh**,
paid with **x402**, and settled **only when the robot actually succeeded**.

| | |
|---|---|
| robotId | `cra-001` |
| profileId | `laok.cra-001.pick-and-stack.v1` |
| skill | `pick_and_stack` — 0.10 USDC / execution, Base Sepolia |
...
```
 ...
 flow/zenoh_transport.py         publish  robot/tunnel/action
      ▼
 flow/node.py                    cra-001 robot node
      ▼
...
`pick_and_stack` runs five stages — `MOVE_ABOVE → DESCEND → GRIP → LIFT → SETTLE`
...
| [`profiles/skills.yaml`](profiles/skills.yaml) | `pick_and_stack` price, params schema, success criteria, failure modes |
...
The same `pick_and_stack` definition runs on two independent engines:
