...
REQ = {"skill": "pick_and_stack", "robotId": "cra-001"}
...
class TestSkillsCatalogMatchesCode(unittest.TestCase):
    ...
    def test_success_thresholds_match_spec(self):
        crit = {c["key"]: c["value"]
                for c in profiles.skill("pick_and_stack")["successCriteria"]}
    ...
    def test_declared_failure_modes_are_the_real_ones(self):
        declared = {f["reason"] for f in profiles.skill("pick_and_stack")["failureModes"]}
        self.assertEqual(declared,
                         {"unreachable", "collision", "timeout", "grasp_failed"})
        for mode in profiles.skill("pick_and_stack")["failureModes"]:
            self.assertFalse(mode["settles"], f"{mode['reason']} must never settle")

    def test_result_schema_matches_build_metrics(self):
        required = set(profiles.skill("pick_and_stack")["resultSchema"]
    ...
    def test_price_is_declared_once_and_is_coherent(self):
        p = profiles.skill("pick_and_stack")["pricing"]
...
class TestPaymentPolicy(unittest.TestCase):
    ...
    def test_no_private_key_literal_anywhere_in_the_bridge(self):
        # On-chain tx hashes are public 64-hex identifiers and legitimately
        # appear in evidence files. Skip lines that only *reference* a tx
        # (tx_hash / txHash / basescan / paymentTx), never a private key.
        TX_REF = ("tx_hash", "txhash", "basescan", "paymenttx", "txs:")
        for path in ROOT.rglob("*"):
            if path.is_dir() or path.suffix not in (".py", ".yaml", ".yml", ".md", ".json"):
                continue
            ...
            for line in text.splitlines():
                ...
                if stripped.startswith("#") or "Env:" in stripped:
                    continue
                if any(k in low for k in TX_REF):
                    continue
                self.assertNotRegex(
...
class TestProfilesDriveTheRelay(unittest.TestCase):
    ...
    def test_402_challenge_carries_the_catalogue_price(self):
        ...
        self.assertEqual(accept["amount"],
                         profiles.skill("pick_and_stack")["pricing"]["amount"])
    ...
    def test_discovery_is_free_and_lists_the_price(self):
        cat = profiles.list_skills("cra-001")
        self.assertEqual(cat["robotId"], "cra-001")
        entry = cat["skills"][0]
        self.assertEqual(entry["skillId"], "pick_and_stack")
    ...
    def test_payto_address_comes_from_the_environment(self):
        ...
        try:
            accepts = profiles.payment_requirements("pick_and_stack")
