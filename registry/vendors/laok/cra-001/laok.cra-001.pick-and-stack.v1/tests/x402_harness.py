"""Local Fabric/x402 harness for cra-001's real Go Tunnel integration tests.
...
try:
    import zenoh
except Exception:  # pragma: no cover - zenoh wheels are Linux-only
...
class InjectedFabricSimulator:
    ...
    def __init__(
        ...
        robot_id: str = "cra-001",
        action_topic: str = "robot/tunnel/action",
        result_topic: str = "robot/tunnel/result",
    ):
        if zenoh is None:  # pragma: no cover - zenoh wheels are Linux-only
            raise unittest.SkipTest("zenoh not importable on this platform")
