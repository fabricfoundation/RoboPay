...
$repoRoot = (& git -C $profileRoot rev-parse --show-toplevel).Trim()
...
if (-not (Test-Path -LiteralPath $TunnelBin)) {
  throw "Tunnel binary was not found: '$TunnelBin'. Run 'wsl.exe -d Ubuntu-22.04 -- make build' from $repoRoot first."
}
...
$env:LIMX_TRON2_MUJOCO_VIEWER_HOLD_SECONDS = [string]$HoldSeconds
$env:ROBO_PAY_COMMIT_SHA = $commitSha

& $python (Join-Path $profileRoot 'bridge\download_vendor_assets.py')
...
public static class RoboPayTron2WindowLayout {
...
"@
  ...
  if ($terminalHandle -ne [IntPtr]::Zero) {
    [void][RoboPayTron2WindowLayout]::MoveWindow(
...
using System;
using System.Runtime.InteropServices;
public static class RoboPayTron2ViewerLayout {
    [DllImport("user32.dll", SetLastError = true)]
    public static extern bool MoveWindow(IntPtr hWnd, int x, int y, int width, int height, bool repaint);
}
"@
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
      ...
      foreach ($viewer in $viewers) {
        [void][RoboPayTron2ViewerLayout]::MoveWindow(
...
Write-Host 'OBS sequence: bridge ready -> discovery -> unpaid 402 -> first paid 202 -> TRON 2 MuJoCo motion -> settlement -> BaseScan'
