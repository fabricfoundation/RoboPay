"""
MuJoCo episode runner for the Deep Robotics M20 Pro obstacle-navigation skill.
...
class M20ProMuJoCoRunner:
    def __init__(self, scene_path: str):
        ...
        self._base_body_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "m20_pro_base"
        )
    ...
    def _apply_leg_gait(self, sim_time: float) -> None:
        ...
        leg_sign = {"fl": 1, "rr": 1, "fr": -1, "rl": -1}
        for leg, sign in leg_sign.items():
            swing = max(math.sin(phase) * sign, 0.0)
            self.data.ctrl[self._actuator_index[f"{leg}_hip"]] = 0.0
            self.data.ctrl[self._actuator_index[f"{leg}_thigh"]] = math.radians(
                5 + 15 * swing
            )
            self.data.ctrl[self._actuator_index[f"{leg}_calf"]] = math.radians(
                -60 - 20 * swing
            )
