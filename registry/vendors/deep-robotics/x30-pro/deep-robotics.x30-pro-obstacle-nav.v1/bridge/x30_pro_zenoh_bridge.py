"""
Deep Robotics X30 Pro — RoboPay Zenoh bridge (Tier 1, simulator-only).

Subscribes to robot/tunnel/action, validates and de-duplicates the envelope,
drives the MuJoCo X30 Pro obstacle-navigation episode, and publishes a
...
logger = logging.getLogger("x30_pro_robopay_bridge")
...
def main() -> None:  # pragma: no cover - wiring only, exercised via tests/mocks
    ...
    runner = X30ProMuJoCoRunner(scene_path=os.environ.get(
        "X30_PRO_SCENE", "simulation/scenes/x30_pro.xml"
    ))
    bridge = X30ProRoboPayBridge(robot_id=robot_id, runner=runner)
    ...
    with zenoh.open(zenoh.Config()) as session:
        ...
        logger.info("x30_pro_robopay_bridge listening on %s", ACTION_TOPIC)
        while True:
            time.sleep(1.0)
