# Deep Robotics X30 Pro — Tier 1 Validation Report

## Summary

This is a simulator-only Tier 1 submission for the X30 Pro obstacle-avoidance
navigation skill. All evidence below was produced by running the included
demo scripts against the real MuJoCo scene at
`simulation/scenes/x30_pro.xml` — no artifacts are reused from another
robot's profile. Robot mass, dimensions, joint limits, and link offsets are
sourced from the official Deep Robotics X30 URDF
(`legubiao/quadruped_ros2_control`), not estimated or copied from another
robot.
...
paid request runs the real X30 Pro MuJoCo episode → replay of the same
...
|--------|-------|
| status | goal_reached |
| displacement_m | 6.6526 |
| path_length_m | 6.6788 |
| collisions (real MuJoCo contacts) | 0 |
| avoidance_events | 2432 |
| sim_steps | 5417 |
| sim_seconds | 10.832 |
| target_distance_remaining_m | 0.3499 |
| settlementEligible | true |

Terminal log: `docs/evidence/terminal_log.txt`
Raw metrics: `docs/evidence/x30_pro_metrics.json`
...
| sim_steps | 500 |
| target_distance_remaining_m | 6.5942 |
| settlementEligible | **false** |

Terminal log: `docs/evidence/failure_terminal_log.txt`
Raw metrics: `docs/evidence/x30_pro_failure_metrics.json`
...
| displacement_within_tolerance | PASS -- mujoco=6.6526 webots=6.6547 diff=0.0021 |
| remaining_distance_within_tolerance | PASS -- mujoco=0.3499 webots=0.3466 diff=0.0033 |
...
- the action targets the published `x30_pro_obstacle_navigation` skill at
...
- the X30 Pro MuJoCo scene (`simulation/scenes/x30_pro.xml`) defines
  actuators specific to this robot's kinematics (12 leg joints + 3 base
  DOF, using the official `FL/FR/HL/HR` + `HipX/HipY/Knee` naming), and
  `simulation/runners/x30_pro_runner.py` reads actuator names dynamically
  rather than hardcoding `ctrl` indices; metrics and logs are all produced
  by running this X30 Pro scene, not reused from another robot's profile;
- the terminal `robot/tunnel/result` uses the same `actionId` as the
  originating action (see `docs/evidence/x30_pro_metrics.json`);
...
The X30 Pro base is mounted on a planar (x, y, yaw) joint rather than a
...
## Physical specification provenance (disclosed)

This submission uses the official Deep Robotics X30 specification
(59 kg, 1000x715x470 mm), cross-checked against the component-level
masses in the official URDF (`legubiao/quadruped_ros2_control`), which
sum to ~55.8 kg excluding battery/electronics — consistent with the
59 kg published spec.
