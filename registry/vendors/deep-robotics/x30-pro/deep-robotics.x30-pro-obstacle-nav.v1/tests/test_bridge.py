"""
Executable contract tests for the X30 Pro RoboPay bridge.
...
def make_action(**overrides):
    params = {"target_xy": [7.0, 0.0], "max_episode_steps": 50000}
    base = {
        "actionId": "action-1",
        "robotId": ROBOT_ID,
        "skillId": SKILL_ID,
...
def test_successful_action_settles():
    runner = FakeRunner()
    bridge = X30ProRoboPayBridge(ROBOT_ID, runner, ReplayStore())
...
def test_timeout_result_is_never_settlement_eligible():
    runner = FakeRunner(next_result={
        ...
        "path_length_m": 0.43,
        "collisions": 0,
        "target_distance_remaining_m": 6.59,
        "sim_steps": 500,
        "sim_seconds": 1.0,
    })
    bridge = X30ProRoboPayBridge(ROBOT_ID, runner, ReplayStore())
