"""
Webots controller for the Deep Robotics X30 Pro obstacle-navigation skill.

Runs the SAME potential-field navigation policy as
simulation/runners/x30_pro_runner.py (MuJoCo), driving a proxy robot body
in Webots instead. This is not a re-implementation of the physical leg
model -- it is a Sim-to-Sim consistency check: the same policy code,
given the same obstacle layout and goal, must produce the same high-level
outcome (goal_reached, zero collisions) regardless of which physics
engine executes it.
"""
...
# Half-extents used for real geometric contact checks (matches world file)
BASE_HALF_EXTENT = (0.22, 0.13)
OBSTACLE_HALF_EXTENT = {
    "obstacle_1": (0.15, 0.15),
    "obstacle_2": (0.15, 0.15),
    "obstacle_3": (0.15, 0.15),  # cylinder radius treated as square half-extent
}
...
def main():
    ...
    for steps in range(max_steps):
        ...
        # Real geometric AABB overlap check against each obstacle (not the
        # SAFE_OBSTACLE_DIST_M heuristic used for avoidance steering above).
