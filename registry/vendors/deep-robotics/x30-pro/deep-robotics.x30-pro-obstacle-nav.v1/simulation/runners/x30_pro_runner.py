"""
MuJoCo episode runner for the Deep Robotics X30 Pro obstacle-navigation skill.
...
actuators). The four legs are independently position-actuated every step
with a real trot-timed gait (not a canned/pre-recorded animation -- the gait
phase is computed live from simulation time each step). Obstacle collisions
are detected using MuJoCo's own narrow-phase contact detection against
base_geom, not a proximity heuristic, so the reported `collisions` metric
reflects genuine physical contact.

Joint/actuator naming follows the official Deep Robotics X30 URDF
convention (legubiao/quadruped_ros2_control): FL/FR/HL/HR legs, each with
HipX/HipY/Knee joints -- distinct from M20 Pro's fl/fr/rl/rr + hip/thigh/calf
naming.
"""
...
# Default standing pose per the official X30 robot_control.yaml (radians)
STANDING_HIPY_RAD = -0.732
STANDING_KNEE_RAD = 1.361
...
class X30ProMuJoCoRunner:
    def __init__(self, scene_path: str):
        ...
        self._base_body_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "x30_pro_base"
        )
    ...
    def _base_yaw(self) -> float:
        # qpos layout: [base_x, base_y, base_yaw, <12 leg joints>]
    ...
    def _apply_leg_gait(self, sim_time: float) -> None:
        """Real, live-computed trot gait -- not a pre-recorded trajectory."""
        phase = 2.0 * math.pi * GAIT_FREQ_HZ * sim_time
        leg_sign = {"FL": 1, "HR": 1, "FR": -1, "HL": -1}
        for leg, sign in leg_sign.items():
            swing = max(math.sin(phase) * sign, 0.0)
            self.data.ctrl[self._actuator_index[f"{leg}_hip_act"]] = 0.0
            self.data.ctrl[self._actuator_index[f"{leg}_thigh_act"]] = STANDING_HIPY_RAD + math.radians(
                15 * swing
            )
            self.data.ctrl[self._actuator_index[f"{leg}_calf_act"]] = STANDING_KNEE_RAD - math.radians(
                20 * swing
            )

    def _navigate_step(self, target_xy: np.ndarray) -> tuple[float, bool]:
        """Compute and apply base velocity command for one step.
        Returns (distance_to_goal, avoidance_triggered)."""
        ...
        return dist_to_goal, avoidance_triggered

    def stop(self) -> None:
        self._stop_requested = True

    def run_episode(self, params: dict) -> dict:
        target_xy = np.array(params.get("target_xy", [7.0, 0.0]), dtype=float)
        ...
        for step in range(max_steps):
            ...
            dist_to_goal, avoidance_triggered = self._navigate_step(target_xy)
            ...
            self._apply_leg_gait(self.data.time)
