"""Policy-driven obstacle navigation for Dobot CRA robot arm.
...
class ObstacleNavSkill(RobotSkill):
    ...
    The robot arm end-effector is attracted to the goal and repelled by obstacles.
    Forces are combined to produce velocity commands.

    CRA arm constraints:
      - No reverse end-effector travel (vx >= 0)
      - max_speed: 0.5 m/s
    ...
    def __init__(
        ...
        self._max_speed = min(max_speed, 0.5)  # CRA hard limit
        self._max_angular = min(max_angular, 0.3)  # CRA hard limit
    ...
    def step(self, state: Dict[str, Any]) -> SkillResult:
        ...
        # CRA arm: forward-only end-effector motion (vx >= 0)
        if abs(angle_error) < 0.3:
            ...
            # Goal is to the left — rotate arm base
            action = "turn_left"
            speed = 0.1  # Minimal forward while turning for arm stability
            angular = min(self._max_angular, 0.5 * angle_error)
        else:
            # Goal is to the right — rotate arm base
            action = "turn_right"
            speed = 0.1
            angular = max(-self._max_angular, 0.5 * angle_error)

        # Slow down near obstacles (arm needs more caution near objects)
        if min_obstacle_dist < 1.0:
            speed *= 0.5

        # CRA cannot reverse end-effector — clamp to non-negative
        speed = max(0.0, speed)
