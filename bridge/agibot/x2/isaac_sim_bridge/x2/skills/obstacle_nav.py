"""Policy-driven obstacle navigation for AGIBot X2 humanoid.
...
class ObstacleNavSkill(RobotSkill):
    ...
    Tuned for AGIBot X2 humanoid dynamics:
      - vx: [0, 1.0] m/s (forward-only locomotion)
      - wz: [-0.3, 0.3] rad/s
    """
    ...
    def step(self, state: Dict[str, Any]) -> SkillResult:
        ...
        # AGIBot X2 is a humanoid: forward-only, wider angular range
        if abs(angle_error) < 0.3:
            ...
            # Goal is to the left
            action = "turn_left"
            speed = 0.2  # Slow forward while turning
