"""Convert the AgiBot X2 description from STL meshes to OBJ for Drake.

Drake computes convex hulls for collision geometry and only accepts .obj,
.vtk, or .gltf; the AgiBot description ships .STL exclusively. MuJoCo reads
`x2_ultra.xml` straight from the same checkout and is unaffected, so this
conversion exists purely to let both engines load the *same* robot.

The output is written to a separate tree so the upstream checkout stays
pristine and the conversion stays reproducible. Run it from the repository
root:

    python bridge/agibot/x2/sim_bridge/tools/convert_meshes.py \
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--src", required=True, type=Path,
                    help="upstream x2 description directory")
    ap.add_argument("--dest", required=True, type=Path,
                    help="output directory for the OBJ-based description")
    ap.add_argument("--urdf", default="x2_ultra_simple_collision.urdf",
