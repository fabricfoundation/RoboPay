...
from .drake_env import DrakeX2Env
from .metrics import RunMetrics, compare
from .runner import TaskRunner, mujoco_factory


def drake_factory(px: float, py: float, gx: float, gy: float) -> DrakeX2Env:
    return DrakeX2Env(puck_x=px, puck_y=py, goal_x=gx, goal_y=gy)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--puck", nargs=2, type=float, default=[0.24, 0.16],
                        metavar=("X", "Y"))
    parser.add_argument("--goal", nargs=2, type=float, default=[0.26, 0.30],
                        metavar=("X", "Y"))
