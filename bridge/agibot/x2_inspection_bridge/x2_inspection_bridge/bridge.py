"""Fail-closed Zenoh execution bridge for the paid AGIBot X2 skill."""
...
LOGGER = logging.getLogger("robopay.agibot_x2")
ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/agibot_x2/metrics"
ROBOT_ID = "agibot-x2-sim-01"
PROFILE_ID = "agibot.x2-ultra.mujoco-webots-active-inspection.v1"
...
def _visual_payment_demo() -> tuple[bool, float | None, float, float]:
    enabled = os.environ.get("AGIBOT_X2_MUJOCO_VIEWER", "").strip().lower() in {"1", "true", "yes"}
    if not enabled:
        return False, None, 0.0, 0.0
    try:
        hold = float(os.environ.get("AGIBOT_X2_MUJOCO_VIEWER_HOLD_SECONDS", "5"))
        target_hold = float(os.environ.get("AGIBOT_X2_TARGET_HOLD_SECONDS", "0"))
        start_hold = float(os.environ.get("AGIBOT_X2_VIEWER_START_HOLD_SECONDS", "0"))
...
class X2ZenohBridge:
    def __init__(self, model_dir: str | None = None, settings: BridgeSettings | None = None):
        ...
        ready_path = os.environ.get("AGIBOT_X2_READY_FILE", "").strip()
        if ready_path:
    ...
    def _execute(self, event, duration: float, targets: tuple[str, ...], speed: float, viewer: bool, hold: float | None, target_hold: float, start_hold: float) -> None:
        try:
            ...
            LOGGER.exception("X2 simulator execution failed")
        ...
        LOGGER.info(
            "X2 action completed action_id=%s status=%s targets_confirmed=%s",
            event.action_id,
    ...
    def _on_action(self, sample) -> None:
        ...
        with self._lock:
            ...
            LOGGER.info(
                "Authorized X2 ActionEvent accepted action_id=%s robot_id=%s targets=%s",
                event.action_id,
                event.robot_id,
                targets,
            )
            self._worker = threading.Thread(target=self._execute, args=(event, duration, targets, speed, viewer, hold, target_hold, start_hold), daemon=True, name=f"x2-action-{event.action_id}")
            self._worker.start()

    def spin(self) -> None:
        LOGGER.info(
            "X2 bridge robot_id=%s listening=%s results=%s metrics=%s",
            self.robot_id,
