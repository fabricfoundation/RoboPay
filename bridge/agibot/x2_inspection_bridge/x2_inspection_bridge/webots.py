"""Launch the native Webots validation for the supported X2 station."""
...
SCENE = PACKAGE_ROOT / "scenes" / "x2_inspection_station_ci.wbt"
VIEWER_SCENE = PACKAGE_ROOT / "scenes" / "x2_inspection_station.wbt"
RESULT = PACKAGE_ROOT / "scenes" / "webots_inspection_result.json"
PROTOS = (PACKAGE_ROOT / "scenes" / "X2Ultra.proto", PACKAGE_ROOT / "scenes" / "X2UltraCI.proto")
...
def run_webots_validation(timeout_seconds: int = 300) -> dict:
    ...
    environment.pop("AGIBOT_X2_WEBOTS_HOLD_VIEWER", None)
...
def launch_webots_viewer() -> int:
    ...
    environment["WEBOTS_CONTROLLER_PATH"] = str(PACKAGE_ROOT / "controllers")
    environment["PYTHONPATH"] = str(PACKAGE_ROOT) + os.pathsep + environment.get("PYTHONPATH", "")
    environment["AGIBOT_X2_WEBOTS_HOLD_VIEWER"] = "1"
    environment.setdefault("WEBOTS_HOME", str(_webots_home(executable)))
