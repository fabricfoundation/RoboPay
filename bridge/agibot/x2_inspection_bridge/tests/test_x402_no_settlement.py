...
ROBOT_ID = "agibot_x2_no_settlement"
PORT = int(os.environ.get("AGIBOT_X2_NO_SETTLE_ZENOH_PORT", "7468"))
...
class InjectedSimulator:
    ...
    def on_action(self, sample):
        ...
        self.publisher.put(json.dumps({
            ...
            "status": "failure", "result": {"error_code": "INJECTED_X2_FAILURE"},
        }).encode())
...
class X2NoSettlementTests(unittest.TestCase):
    def test_failure_timeout_and_replay_never_settle(self):
        ...
        try:
            proxy.start()
            with tempfile.TemporaryDirectory(prefix="x2_nosettle_") as temporary:
                ...
                failed_id = f"x2-failure-{uuid.uuid4().hex}"
                ...
                simulator.mode = "silent"; timeout_id = f"x2-timeout-{uuid.uuid4().hex}"; timeout_signature = payment_signature_from_402(headers)
                ...
                payment_replay_id = f"x2-payment-replay-{uuid.uuid4().hex}"
