...
from x2_inspection_bridge.bridge import X2ZenohBridge, _load_event_parser
...
def event(action="inspect_target_sequence", params=None, action_id="action-1"):
    ...
    return {
        ...
        "robot_id": "x2-contract-test", "skill_id": action, "idempotency_key": action_id,
...
def bridge():
    item = X2ZenohBridge.__new__(X2ZenohBridge)
    item.robot_id = "x2-contract-test"; item._model_dir = None; item._parse = _load_event_parser()
...
class X2BridgeContractTests(unittest.TestCase):
    ...
    def test_success_and_failure_are_correlated(self):
        for simulator_result, status in (({"success": True}, "success"), ({"success": False}, "failure")):
            item = bridge()
            with patch("x2_inspection_bridge.bridge.run_inspection", return_value=simulator_result):
                item._on_action(_Sample(event())); item._worker.join(2)
            result = item._results.documents[-1]
            self.assertEqual(result["status"], status)
            self.assertEqual((result["action_id"], result["robot_id"], result["skill_id"], result["idempotency_key"]), ("action-1", "x2-contract-test", "inspect_target_sequence", "action-1"))
    ...
    def test_invalid_contracts_never_run_simulator(self):
        ...
        for document, code in cases:
            item = bridge()
            with patch("x2_inspection_bridge.bridge.run_inspection") as runner:
    ...
    def test_paid_stop_interrupts_and_confirms_safe_stop(self):
        ...
        with patch("x2_inspection_bridge.bridge.run_inspection", side_effect=runner):
