"""Zenoh bridge for paid cra-001 ``pick_and_stack`` actions.
...
see already-paid actions, runs the real MuJoCo physics for ``pick_and_stack``,
...
LOGGER = logging.getLogger("robopay.cra")
...
ROBOT_ID = "cra-001"
ALLOWED_ACTIONS = {"pick_and_stack"}
PROFILE_ID = "laok.cra-001.pick-and-stack.v1"
...
class FabricZenohBridge:
    ...
    def _on_action(self, sample) -> None:
        ...
        try:
            res = self._executor.execute("pick_and_stack", params)
    ...
    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "cra-001 bridge %s listening on %s; publishing results on %s",
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the cra-001 bridge as a standalone Zenoh worker."""
