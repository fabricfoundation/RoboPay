"""x402 payment verification for cra-001 (D7 payment boundary).
...
class X402Verifier:
    ...
    def verify(self, payment: dict, challenge: Optional[X402Challenge] = None) -> dict:
        challenge = challenge or X402Challenge("pick_and_stack")
        if not payment:
...
def verify_payment(payment: dict | None) -> dict:
    """Verify a payment receipt against the pick_and_stack x402 challenge.
