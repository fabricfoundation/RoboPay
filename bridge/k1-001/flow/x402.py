"""x402 payment verification for k1-001 (D7 payment boundary).
...
class X402Verifier:
    ...
    def verify(self, payment: dict, challenge: Optional[X402Challenge] = None) -> dict:
        challenge = challenge or X402Challenge("active_inspection")
        if not payment:
...
def verify_payment(payment: dict | None) -> dict:
    """Verify a payment receipt against the active_inspection x402 challenge.
