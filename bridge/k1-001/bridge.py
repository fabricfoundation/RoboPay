"""Zenoh bridge for paid k1-001 ``active_inspection`` actions.
...
see already-paid actions, runs the real MuJoCo physics for ``active_inspection``,
...
This is the fix for the reviewer's CHANGES_REQUESTED note:
settlement now flows through the real Tunnel + x402 facilitator.
...
LOGGER = logging.getLogger("robopay.k1")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/k1-001/metrics"

ROBOT_ID = "k1-001"
ALLOWED_ACTIONS = {"active_inspection"}
PROFILE_ID = "laok.k1-001.active-inspection.v1"
...
KNOWN_SCENARIOS = {
    "inspection", "full_inspection", "three_targets",
    "timeout", "fast_inspection", "single_target", "one_target",
}
...
def _validate_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    unexpected = sorted(set(params) - {"scenario", "maxSteps"})
    if unexpected:
        raise ActionContractError(
            "INVALID_PARAMS", f"unregistered parameter(s): {', '.join(unexpected)}"
        )
    scenario = params.get("scenario", "inspection")
    if scenario not in KNOWN_SCENARIOS:
        raise ActionContractError("INVALID_PARAMS", f"unknown scenario: {scenario!r}")
...
class FabricZenohBridge:
    ...
    def _on_action(self, sample) -> None:
        ...
        try:
            res = self._executor.execute("active_inspection", params)
    ...
    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "k1-001 bridge %s listening on %s; publishing results on %s",
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the k1-001 bridge as a standalone Zenoh worker."""
