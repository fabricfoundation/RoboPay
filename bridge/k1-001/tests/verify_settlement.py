...
def main():
    ...
    for tx_hash in hashes:
        # A non-canonical placeholder (0xPENDING_...) means the evidence file
        # is awaiting its robot-specific on-chain tx before the PR is opened.
        # Skip it explicitly instead of spraying a malformed hash at the RPC.
        if len(tx_hash) != 66 or not tx_hash.startswith("0x") \
                or any(ch not in "0123456789abcdefABCDEF" for ch in tx_hash[2:]):
            pending += 1
            print("SKIP %s (pending placeholder, mint real tx before push)"
    ...
    print("VERIFIED %d/%d settlement tx(s) on Base Sepolia "
          "(failed: %d, network-unreachable: %d, pending: %d)"
          % (verified, len(hashes), len(failed_hashes), unreachable, pending))
