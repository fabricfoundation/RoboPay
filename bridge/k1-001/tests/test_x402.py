"""D7 payment-boundary tests --- x402 protocol verification.
...
class TestRelayOnlyDispatchesVerifiedPayments(unittest.TestCase):
    ...
    def _req(self, key, **over):
        req = {"skill": SKILL, "robotId": ROBOT, "idempotencyKey": key}
        req.update(over)
        return req

    def test_unpaid_is_402_no_execution(self):
        r, ex = self._relay()
        resp = r.handle(self._req("k-u1"))
        self.assertEqual(resp["status"], 402)
        self.assertEqual(ex.execution_count, 0)

    def test_bad_amount_is_402_no_execution(self):
        r, ex = self._relay()
        resp = r.handle(self._req("k-u2", payment=valid_receipt(amount="0.99")))
        self.assertEqual(resp["status"], 402)
        self.assertEqual(ex.execution_count, 0)

    def test_malformed_txhash_is_402_no_execution(self):
        r, ex = self._relay()
        resp = r.handle(self._req("k-u3", payment=valid_receipt(tx_hash="0xzzz")))
        self.assertEqual(resp["status"], 402)
        self.assertEqual(ex.execution_count, 0)

    def test_verified_payment_executes_and_settles(self):
        r, ex = self._relay()
        resp = r.handle(self._req("k-ok", payment=valid_receipt(),
                                  params={"scenario": "inspection"}))
    ...
    def test_replay_of_verified_payment_is_rejected_no_double_settle(self):
        r, ex = self._relay()
        first = r.handle(self._req("k-r1", payment=valid_receipt(),
                                   params={"scenario": "inspection"}))
        self.assertTrue(first["settled"])
        replay = r.handle(self._req("k-r2", payment=valid_receipt(),
                                    params={"scenario": "inspection"}))
...
class TestRealMuJoCoCorrelated(unittest.TestCase):
    ...
    def test_real_mujoco_inspection_succeeds(self):
        ex = MuJoCoExecutor()
        res = ex.execute(SKILL, {"scenario": "inspection"})
        self.assertTrue(res.success, msg=f"mujoco sim failed: {res.message}")
        self.assertEqual(res.message, "all_targets_confirmed")
        self.assertIs(res.metrics.get("fovCentered"), True)
