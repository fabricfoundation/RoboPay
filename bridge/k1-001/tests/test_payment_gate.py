"""Exercise cra-001's x402 payment gate through the real Go Tunnel binary.
...
ROBOT_ID = "cra_arm_001_payment_gate"
...
class FabricPaymentGateTests(unittest.TestCase):
    def test_unpaid_and_invalid_payment_fail_closed(self) -> None:
        ...
        try:
            proxy.start()
            with tempfile.TemporaryDirectory(prefix="cra_payment_gate_") as temp_dir:
                temp = Path(temp_dir)
                ...
                unpaid_status, unpaid_headers, unpaid_body = http_post(
                    action_url, {"action": "pick_and_stack"}
                )
                ...
                # Prove the 402 challenge is wired for cra-001.
                required = json.loads(
                ...
                malformed_status, _, _ = http_post(
                    action_url, {"action": "pick_and_stack", "params": "not-an-object"}
                )
                ...
                tampered_id = f"cra-tampered-payment-{uuid.uuid4().hex}"
