"""Policy-driven obstacle navigation for LimX TRON2 humanoid.
...
TRON2 is a humanoid robot with the following constraints:
  - Forward only: vx in [0, 0.8] m/s (no backward walking)
  - Angular:      wz in [-0.3, 0.3] rad/s
...
class ObstacleNavSkill(RobotSkill):
    ...
    TRON2 humanoid constraints:
      - No backward walking (vx >= 0)
      - max_speed: 0.8 m/s
    ...
    def __init__(
        ...
        self._max_speed = min(max_speed, 0.8)  # TRON2 hard limit
        self._max_angular = min(max_angular, 0.3)  # TRON2 hard limit
    ...
    def step(self, state: Dict[str, Any]) -> SkillResult:
        ...
        # TRON2 is a humanoid: forward-only locomotion (vx >= 0)
        if abs(angle_error) < 0.3:
            ...
            # Goal is to the left -- turn in place (humanoid pivot)
            action = "turn_left"
            speed = 0.15  # Minimal forward while turning for humanoid stability
            angular = min(self._max_angular, 0.5 * angle_error)
        else:
            # Goal is to the right -- turn in place
            action = "turn_right"
            speed = 0.15
            angular = max(-self._max_angular, 0.5 * angle_error)

        # Slow down near obstacles (humanoids need more caution)
        if min_obstacle_dist < 1.0:
            speed *= 0.5

        # TRON2 cannot walk backward -- clamp to non-negative
        speed = max(0.0, speed)
