...
class Tron2Mapper(CommandMapper):
    """Maps Fabric actions to Twist commands for LimX TRON2 humanoid.
    ...
    def map(self, event: ActionEvent) -> Twist:
        ...
        TRON2 is a humanoid: no backward walking (vx >= 0).
        """
        msg = Twist()
        a = event.action
        ...
        if a in ("move_forward", "forward"):
            ...
            # TRON2 humanoid: backward not supported -- issue stop instead
            pass  # zero Twist (safe stop)
        elif a == "turn_left":
    ...
    def skill_result_to_twist(self, action: str, speed: float, angular: float) -> Twist:
        ...
        if action == "move_forward":
            msg.linear.x = clamp(speed, 0.0, 0.8)
