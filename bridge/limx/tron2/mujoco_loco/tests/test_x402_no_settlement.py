"""Proof that failed / timed-out / replayed limx-tron2 actions never call the
x402 settle path.
...
class TestUnpaidRejected402(unittest.TestCase):
    ...
    def test_unpaid_is_402_no_settle(self):
        ex = MockExecutor()
        r = Relay(ex)
        resp = r.handle({"skill": "move_forward", "robotId": "limx-tron2",
...
class TestReplayRejected409(unittest.TestCase):
    ...
    def test_replay_rejected_no_double_settle(self):
        ex = MockExecutor()
        r = Relay(ex)
        first = r.handle({"skill": "move_forward", "robotId": "limx-tron2",
                          "idempotencyKey": "ns-r1", "payment": valid_receipt()})
        self.assertTrue(first["settled"])
        replay = r.handle({"skill": "move_forward", "robotId": "limx-tron2",
