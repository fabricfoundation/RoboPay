# limx-tron2 -- RoboPay Tier 1 (Simulator Skill Execution)

Planar biped walker for **TRON 2** (LimX Dynamics), executed by a real
MuJoCo physics engine. This bridge targets the official bounty branch
`limx-tron2-tier-1` -- the PR is opened against that branch, not `main` (the #90 lesson).

## What is real
- **Distinct morphology**: link lengths / leg count / gait cadence differ from
  every other robot in the prize pool (see `engine.py::ROBOTS["limx-tron2"]`).
