"""Policy-driven obstacle navigation for LimX TRON1 humanoid.
...
TRON1 is a humanoid robot with the following constraints:
...
class ObstacleNavSkill(RobotSkill):
    ...
    def __init__(
        ...
        self._max_speed = min(max_speed, 0.8)  # TRON1 hard limit
        self._max_angular = min(max_angular, 0.3)  # TRON1 hard limit
    ...
    def step(self, state: Dict[str, Any]) -> SkillResult:
        ...
        # TRON1 is a humanoid: forward-only locomotion (vx >= 0)
        ...
        # TRON1 cannot walk backward — clamp to non-negative
        speed = max(0.0, speed)
