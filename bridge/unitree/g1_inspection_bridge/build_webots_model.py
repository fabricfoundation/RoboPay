"""Build full-detail viewer and lightweight CI PROTOs from Unitree's G1 URDF."""
...
from download_g1_model import LOCK_PATH, download


HERE = Path(__file__).resolve().parent
FULL_OUTPUT = HERE / "scenes" / "G129dof.proto"
CI_OUTPUT = HERE / "scenes" / "G129dofCI.proto"
OUTPUT = FULL_OUTPUT
WEBOTS_MODEL = HERE / "models" / "unitree_g1_webots"
...
def _build_optimized_model(source: Path) -> Path:
    ...
    marker_value = f'{lock["webotsCommit"]}:faces={MAX_FACES_PER_LINK}'
    ...
    shutil.copy2(source.parent / "LICENSE-unitree_ros", WEBOTS_MODEL / "LICENSE")
...
def _fix_root_physics(output: Path) -> None:
    ...
    content = content.replace("# license: Apache License 2.0", "# license: BSD-3-Clause")
    content = content.replace(
        "# license url: http://www.apache.org/licenses/LICENSE-2.0",
        "# license url: https://github.com/unitreerobotics/unitree_ros/blob/daadf41ee9afce8f90fdc09a98506012691fa122/LICENSE",
    )
...
def build() -> Path:
    ...
    _convert(official_urdf, FULL_OUTPUT, "../models/unitree_g1/webots/")
    # Automated CI uses the same URDF state model with visual-only derivatives.
    _convert(optimized_urdf, CI_OUTPUT, "../models/unitree_g1_webots/")
    return FULL_OUTPUT
