"""Run the Unitree G1 active-inspection episode in MuJoCo."""
...
from .environment import G1InspectionEnvironment, MODEL_COMMIT
from .policy import G1InspectionPolicy
...
def run_inspection(
    ...
    environment = G1InspectionEnvironment(model_dir)
    observation = environment.reset()
    initial = observation
    policy = G1InspectionPolicy(targets, speed_scale)
    policy.reset()
    ...
    return {
        ...
        "robot_model": "Unitree G1 29-DoF (official unitree_mujoco MJCF)",
        ...
        "waist_final_yaw_rad": round(float(observation["joint_positions"][12]), 4),
        "waist_final_pitch_rad": round(float(observation["joint_positions"][14]), 4),
