...
from ..policy.controller import G1PushToTargetPolicy, PolicyConfig
...
from .mujoco_env import MujocoG1Env
...
def mujoco_factory(px: float, py: float, gx: float, gy: float) -> SimEnv:
    return MujocoG1Env(target_x=px, target_y=py, goal_x=gx, goal_y=gy)
