...
from ..g1.action_contract import ActionEnvelope, ActionRejected, RejectionCode, canonical_params_hash
from ..g1.mapper import MAX_PUSH, MIN_PUSH, SKILLS, TaskSpec, catalogue, resolve
...
def test_resolves_valid_parameters():
    ...
    assert task.puck_xy == (0.34, -0.20)
    assert task.goal_xy == (0.44, -0.04)
...
def test_rejects_a_push_that_is_too_short():
    ...
    # first and this stops testing the distance rule at all.
    params = {"puck_x": 0.34, "puck_y": -0.18, "goal_x": 0.36, "goal_y": -0.17}
    with pytest.raises(ActionRejected) as exc:
        resolve(envelope(params=params))
    assert exc.value.code == RejectionCode.PARAMS_OUT_OF_RANGE
