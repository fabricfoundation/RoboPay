"""Launch the actual Webots R2025a obstacle-nav validation."""
...
SCENE = PACKAGE_ROOT / "scenes" / "go2_obstacle_nav.wbt"
RESULT = PACKAGE_ROOT / "scenes" / "webots_obstacle_nav_result.json"
...
def run_webots_validation(timeout_seconds: int = 150) -> dict:
    ...
    environment.pop("GO2_WEBOTS_HOLD_VIEWER", None)
...
def launch_webots_viewer() -> int:
    ...
    # inspect the Go2 and obstacle layout before closing Webots.
    environment["GO2_WEBOTS_HOLD_VIEWER"] = "1"
    process = subprocess.Popen(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Go2's Webots cross-engine validation.")
