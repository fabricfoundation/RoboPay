"""Pay for one live Go2 obstacle-nav action through the public Gateway.

This is an operator demo tool, not a CI test. It assumes the Zenoh router,
Go2 bridge, and Tunnel are already running in separate terminals. The payer
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pay for one live Go2 MuJoCo obstacle-nav action.")
    parser.add_argument("--robot-id", default=os.environ.get("ROBOT_ID", "go2-mujoco-sim-01"))
    ...
    if not 0.5 <= args.speed_scale <= 1.0:
        raise SystemExit("--speed-scale must be between 0.5 and 1.0.")
    ...
    request_id = f"go2-obs-demo-{int(time.time())}"
    ...
    if selected is None or selected.get("price_usdc") != "0.001" or not selected.get("enabled"):
        raise RuntimeError(f"Published Go2 skill is unavailable or has price drift: {discovery}")
