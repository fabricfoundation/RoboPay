"""Real Tunnel proof that failed Go2 actions never call the x402 settle endpoint."""
...
ROBOT_ID = "go2_mujoco_nosettle"
ZENOH_TEST_PORT = int(os.environ.get("GO2_TEST_ZENOH_PORT", "7447"))


class InjectedGo2Simulator:
            ...
            json.dumps(
                {
                    ...
                    "result": {"error_code": "INJECTED_GO2_FAILURE"},
                }
            ).encode()
        )
...
class Go2NoSettlementTests(unittest.TestCase):
    def test_failure_timeout_and_replay_never_settle(self) -> None:
        ...
        try:
            proxy.start()
            with tempfile.TemporaryDirectory(prefix="go2_nosettle_") as temp_dir:
                temp = Path(temp_dir)
                    ...
                    child_env.update(
                        {
                            ...
                            "ALLOWED_ACTIONS": "navigate_obstacles",
                ...
                simulator = InjectedGo2Simulator()
                tunnel = start_tunnel()
                time.sleep(1)
                ...
                failed_id = f"go2-nosettle-failure-{uuid.uuid4().hex}"
                failed_body = {
                ...
                timeout_id = f"go2-nosettle-timeout-{uuid.uuid4().hex}"
                ...
                payment_replay_id = f"go2-nosettle-payment-replay-{uuid.uuid4().hex}"
                status, _, body = http_post(
                    action_url,
                    {
                        "action": "navigate_obstacles",
                ...
                self.assertEqual(len(simulator.actions), 2)
                self.assertEqual(len(settle_calls()), 0)
                print("[GO2 NO-SETTLE] failure, timeout and replay: settle_calls=0")
        finally:
