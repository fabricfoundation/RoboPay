"""Go2-specific Fabric action -> geometry_msgs/Twist mapper.
...
class Go2Mapper(CommandMapper):
    """Maps Fabric actions to Twist commands for Unitree Go2.

    Velocity limits:
      vx: [0, 1.5] m/s   wz: [-1.0, 1.0] rad/s

    Supports:
      - Direct locomotion: move_forward, move_backward, turn_left, turn_right, stop
    ...
    def skill_result_to_twist(self, action: str, speed: float, angular: float) -> Twist:
        ...
        if action == "move_forward":
            msg.linear.x = clamp(speed, 0.0, 1.5)
            msg.angular.z = clamp(angular, -1.0, 1.0)
        elif action == "turn_left":
            msg.linear.x = clamp(speed, 0.0, 1.5)
            msg.angular.z = clamp(angular, 0.0, 1.0)
        elif action == "turn_right":
            msg.linear.x = clamp(speed, 0.0, 1.5)
            msg.angular.z = clamp(angular, -1.0, 0.0)
        elif action == "stop":
            pass  # zero Twist
        return msg
