"""Policy-driven obstacle navigation for Deep Robotics M20 Pro quadruped.
...
class ObstacleNavSkill(RobotSkill):
    ...
    M20 Pro quadruped constraints:
      - Can walk backward (vx can be negative)
      - max_speed: 1.5 m/s
    ...
    def __init__(
        ...
        self._max_speed = min(max_speed, 1.5)  # M20 Pro hard limit
        self._max_angular = min(max_angular, 1.0)  # M20 Pro hard limit
    ...
    def step(self, state: Dict[str, Any]) -> SkillResult:
        ...
        if abs(angle_error) < 0.3:
            ...
            action = "move_forward"
            speed = min(self._max_speed, 0.5 + 0.5 * (1.0 - abs(angle_error)))
            angular = 0.0
        elif abs(angle_error) > 2.5:
            # Goal is roughly behind — walk backward (quadruped capability)
            action = "move_backward"
            speed = min(self._max_speed * 0.5, 0.5)
            angular = 0.0
        elif angle_error > 0:
            # Goal is to the left
            action = "turn_left"
            speed = 0.3  # Moderate forward while turning
            angular = min(self._max_angular, 0.6 * angle_error)
        else:
            # Goal is to the right
            action = "turn_right"
            speed = 0.3
            angular = max(-self._max_angular, 0.6 * angle_error)
