"""Validation tests for Deep Robotics X30 Pro obstacle navigation skill.

Verifies that the obstacle navigation policy respects X30 Pro velocity constraints:
  - vx ∈ [0, 1.5] m/s (forward navigation)
  - wz ∈ [-1.0, 1.0] rad/s
"""
import sys
import os
...
_base_path = os.path.join(os.path.dirname(__file__), "..", "x30_pro", "skills", "base.py")
_spec_base = importlib.util.spec_from_file_location("x30_pro_skills_base", _base_path)
_mod_base = importlib.util.module_from_spec(_spec_base)
_spec_base.loader.exec_module(_mod_base)

_nav_path = os.path.join(os.path.dirname(__file__), "..", "x30_pro", "skills", "obstacle_nav.py")
_spec_nav = importlib.util.spec_from_file_location("x30_pro_skills_obstacle_nav", _nav_path)
_mod_nav = importlib.util.module_from_spec(_spec_nav)
# Patch the base module into the nav module's namespace
_mod_nav.__package__ = "x30_pro.skills"
sys.modules["x30_pro.skills.base"] = _mod_base
...
def _make_skill_and_navigate(goal=(5.0, 0.0), obstacles=None, heading=0.0):
    ...
    skill = ObstacleNavSkill(max_speed=1.5, max_angular=1.0)
...
def test_obstacle_nav_vx_within_max():
    """All navigation commands must have vx <= 1.5 m/s."""
    commands = _make_skill_and_navigate(goal=(5.0, 0.0))
    for i, cmd in enumerate(commands):
        assert cmd.speed <= 1.5, f"step {i}: speed={cmd.speed} > 1.5"


def test_obstacle_nav_vx_non_negative_for_forward():
    """Forward navigation commands must have speed >= 0."""
    commands = _make_skill_and_navigate(goal=(5.0, 0.0))
    for i, cmd in enumerate(commands):
...
def test_obstacle_nav_wz_within_bounds():
    """All navigation commands must have wz in [-1.0, 1.0]."""
    commands = _make_skill_and_navigate(goal=(3.0, 3.0))
    for i, cmd in enumerate(commands):
        assert -1.0 <= cmd.angular_speed <= 1.0, \
            f"step {i}: angular_speed={cmd.angular_speed} out of [-1.0, 1.0]"
...
def test_obstacle_nav_avoids_obstacle():
    ...
    for i, cmd in enumerate(commands):
        assert 0.0 <= cmd.speed <= 1.5, f"step {i}: speed={cmd.speed}"
        assert -1.0 <= cmd.angular_speed <= 1.0, f"step {i}: wz={cmd.angular_speed}"
...
def test_obstacle_nav_quadruped_backward_capability():
    """X30 Pro can walk backward when goal is behind — verify negative speed possible."""
    skill = ObstacleNavSkill(max_speed=1.5, max_angular=1.0)
    skill.reset({"goal": (-5.0, 0.0), "start": (0.0, 0.0), "obstacles": []})
    result = skill.step({"position": (0.0, 0.0), "heading": 0.0})
    # Goal is directly behind — should attempt backward or turn
    assert result.action in ("move_backward", "turn_left", "turn_right", "stop"), \
        f"Unexpected action for behind goal: {result.action}"


def test_obstacle_nav_metrics_structure():
    """Verify metrics dict has expected keys."""
    skill = ObstacleNavSkill()
    skill.reset({"goal": (5.0, 5.0), "start": (0.0, 0.0), "obstacles": []})
    result = skill.step({"position": (0.0, 0.0), "heading": 0.0})
    expected_keys = {"position", "target", "distance_to_goal", "path_progress",
                     "collision_count", "obstacle_clearance", "step_count", "status"}
