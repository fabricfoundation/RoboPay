"""Mandatory real-Tunnel regression for facilitator-rejected M20 payments."""
...
ZENOH_PORT = int(os.environ.get("M20_PAYMENT_GATE_ZENOH_PORT", "7457"))
...
class M20PaymentGateTests(unittest.TestCase):
    def test_websocket_reader_reassembles_continuation_frames(self) -> None:
        ...
        try:
            writer.sendall(
                _frame(b'{"id":"m20-paid",', opcode=1, final=False)
                + _frame(b'"status":202}', opcode=0, final=True)
            )
            opcode, payload = TunnelConnection(reader)._read_message()
            self.assertEqual(opcode, 1)
            self.assertEqual(json.loads(payload), {"id": "m20-paid", "status": 202})
        finally:
            reader.close()
            writer.close()
    ...
    def test_is_valid_false_returns_402_before_action_or_simulator_boundary(self) -> None:
        ...
        try:
            ...
            proxy.start()
            with tempfile.TemporaryDirectory(prefix="m20_payment_gate_") as temp_dir:
                temp = Path(temp_dir)
                ...
                action_id = f"m20-tampered-{uuid.uuid4().hex}"
                ...
                print("[M20 PAYMENT GATE] isValid:false -> 402, 0 actions, 0 simulator output, 0 settlements")
        finally:
