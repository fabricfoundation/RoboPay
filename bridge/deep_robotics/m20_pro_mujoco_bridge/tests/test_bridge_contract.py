...
from m20_pro_mujoco_bridge.contracts import DRIVE_SKILL, ROBOT_ID
...
class M20BridgeContractTests(unittest.TestCase):
    def _bridge(self, runner):
        ...
        with patch("m20_pro_mujoco_bridge.bridge._open_zenoh_session", return_value=session):
            bridge = M20ZenohBridge(settings=settings, episode_runner=runner)
    ...
    def test_bridge_refuses_an_unregistered_robot_identity(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "profile-scoped"):
            M20ZenohBridge(settings=settings, episode_runner=lambda *_args, **_kwargs: {})

    @staticmethod
    def _event(action: str, skill_id: str | None = None, params: dict | None = None):
        return SimpleNamespace(
            action_id="m20-contract-test",
            robot_id=ROBOT_ID,
            action=action,
            skill_id=skill_id or action,
            params=params if params is not None else {},
            params_hash="params-hash",
            idempotency_key="idem-m20-contract-test",
        )
    ...
    def test_invalid_unknown_and_wrong_robot_never_run_simulator(self) -> None:
        ...
        try:
            ...
            bridge._parse_action_event = lambda _payload: SimpleNamespace(
                **{**self._event(DRIVE_SKILL, params={"goalDistanceM": 1.35}).__dict__, "robot_id": "other"}
            )
            bridge._on_action(SimpleNamespace(payload=SimpleNamespace(to_bytes=lambda: b"ignored")))
    ...
    def test_valid_action_preserves_correlation_and_publishes_runner_result(self) -> None:
        ...
        def runner(request, **_kwargs):
            ...
            finished.set()
            return {"success": True, "measured_forward_distance_m": 1.35}

        bridge, session = self._bridge(runner)
        try:
            event = self._event(
                DRIVE_SKILL,
                params={"goalDistanceM": 1.35, "wheelSpeedRadS": 4.0, "maxDurationSec": 16.0},
            )
