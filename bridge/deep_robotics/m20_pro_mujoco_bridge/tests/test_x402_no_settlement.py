"""Real Tunnel proof that M20 failure, timeout, and replay never settle x402.

The Tunnel/x402 middleware and Zenoh transport are production code.  The
``ControlledM20ResultPeer`` is deliberately only a terminal simulator-fault
...
ZENOH_PORT = int(os.environ.get("M20_NO_SETTLEMENT_ZENOH_PORT", "7458"))


class ControlledM20ResultPeer:
            ...
            json.dumps(
                {
                    ...
                    "result": {
                        "success": False,
                        "error_code": "INJECTED_M20_SIMULATOR_FAILURE",
                    },
                },
                separators=(",", ":"),
            ).encode()
...
class M20NoSettlementTests(unittest.TestCase):
    def test_failure_timeout_payment_replay_and_restart_replay_never_settle(self) -> None:
        ...
        try:
            proxy.start()
            simulator = ControlledM20ResultPeer()
            with tempfile.TemporaryDirectory(prefix="m20_no_settlement_") as temp_dir:
                temp = Path(temp_dir)
                ...
                failed_id = f"m20-failure-{uuid.uuid4().hex}"
                ...
                timeout_id = f"m20-timeout-{uuid.uuid4().hex}"
                ...
                payment_replay_status, _, payment_replay_body = http_post(
                    action_url,
                    request_body(f"m20-payment-replay-{uuid.uuid4().hex}"),
                ...
                self.assertEqual(simulator.action_count(), 2)
                self.assertEqual([call for call in FacilitatorHandler.calls if call[0] == "/settle"], [])
                print("[M20 NO-SETTLE] failure, timeout, payment replay, restart replay: settle_calls=0")
        finally:
