"""Fetch the pinned, vendor-published DeepRobotics X30 model."""
...
DEFAULT_DESTINATION = HERE / "models" / "deep_robotics_x30"
...
def download(destination: Path = DEFAULT_DESTINATION) -> Path:
    """Download exactly the locked X30 source tree and return its directory."""
    ...
    with tempfile.TemporaryDirectory(prefix="robopay-x30-model-") as temp_dir:
        ...
        if not _is_complete(source, lock):
            raise RuntimeError("Pinned X30 model did not match model.lock.json")
...
if __name__ == "__main__":
    print(f"DeepRobotics X30 model downloaded to: {download()}")
