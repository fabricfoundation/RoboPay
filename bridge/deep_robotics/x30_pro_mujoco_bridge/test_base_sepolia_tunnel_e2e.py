"""Live Base Sepolia x402 -> Tunnel -> X30 MuJoCo settlement proof.
...
from x30_pro_mujoco_bridge.bridge import READY_TOPIC, RESULT_TOPIC
...
from x30_pro_mujoco_bridge.contracts import MAX_DURATION_SECONDS
...
def _wait_for_bridge_ready(ready: threading.Event, bridge: subprocess.Popen[str]) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if bridge.poll() is not None:
            raise RuntimeError(
                f"X30 bridge exited before declaring its Zenoh action subscription (exit={bridge.returncode})"
            )
    raise RuntimeError("X30 bridge never declared ready; refusing to send the first paid action")
...
def _wait_for_terminal_status(status_url: str, *, timeout_seconds: float = 180.0) -> dict:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(2)
    raise RuntimeError(f"X30 action did not reach terminal status: {last_issue}")


def _wait_for_correlated_result(
    ...
    *,
    ...
    if not ready.wait(timeout_seconds):
        raise RuntimeError("correlated robot/tunnel/result was not observed after execution")
    if len(captured) != 1:
        raise RuntimeError(f"expected exactly one correlated robot result, got {len(captured)}")
    return captured[0]
...
def _stream_tunnel_output(
    ...
    worker = threading.Thread(target=copy_output, name="x30-tunnel-log", daemon=True)
    worker.start()
    return worker
...
def main(argv: list[str] | None = None) -> int:
    ...
    action_id = f"x30-drive-{int(time.time())}"
    action_body = {
    ...
    print(
        f"Course: {course_spec()['course_id']} ({course_fingerprint()})",
        flush=True,
    )

    with tempfile.TemporaryDirectory(prefix="robopay_x30_base_sepolia_") as temp_dir:
        temp = Path(temp_dir)
        ...
        def on_result(sample) -> None:
            try:
                ...
                return
            if payload.get("robot_id") != ROBOT_ID or payload.get("action_id") != action_id:
                return
            correlated_results.append(payload)
            result_ready.set()

        result_subscriber = ready_session.declare_subscriber(RESULT_TOPIC, on_result)
        ...
        bridge_env.update(
            {
                ...
                "X30_MUJOCO_VIEWER": "true" if args.visual else "false",
                "X30_MUJOCO_VIEWER_HOLD_SECONDS": os.environ.get(
                    "X30_MUJOCO_VIEWER_HOLD_SECONDS", "5"
                ),
            }
        )
        bridge = subprocess.Popen(
            [sys.executable, "-m", "x30_pro_mujoco_bridge.bridge"],
            cwd=PACKAGE_ROOT,
            env=bridge_env,
        ...
        try:
            ...
            if {item.get("skill_id") for item in discovery.get("skills", [])} != {"perform_inspection_gait", "stop"}:
            ...
            print("Robot discovery: Deep Robotics X30 Pro simulator-only", flush=True)
            print("Skill discovery: perform_inspection_gait, stop @ 0.001 USDC", flush=True)
            ...
            viewer_hold_seconds = max(
                0.0, float(os.environ.get("X30_MUJOCO_VIEWER_HOLD_SECONDS", "5"))
            )
            required_authorization_window = int(
                MAX_DURATION_SECONDS + viewer_hold_seconds + 0.999
            )
            if authorization_window < required_authorization_window:
                raise RuntimeError(
                    ...
                    f"offered={authorization_window}s, required={required_authorization_window}s"
                )
            print(
                "Payment authorization window: "
                f"{authorization_window}s (required >= {required_authorization_window}s)",
                flush=True,
            )
            if args.dry_run:
                print(
            ...
            if terminal.get("state") != "succeeded" or not terminal.get("settled"):
                raise RuntimeError(f"X30 execution or settlement failed: {terminal}")
            robot_result = _wait_for_correlated_result(result_ready, correlated_results)
            simulation = robot_result.get("result") or {}
            result_course = simulation.get("course") or {}
            if (
                robot_result.get("status") != "success"
                or simulation.get("success") is not True
                or simulation.get("task_goal_reached") is not True
                or simulation.get("course_id") != course_spec()["course_id"]
                or simulation.get("course_hash") != course_fingerprint()
                or float(simulation.get("body_forward_progress_m", 0.0)) < MIN_FORWARD_PROGRESS_M
                or simulation.get("finish_line_crossed") is not True
                or float(result_course.get("finish_line_x_m", float("inf"))) != FINISH_LINE_X_M
                or result_course.get("obstacle_collision_observed") is not False
            ):
                raise RuntimeError(f"correlated simulator result failed the canonical course contract: {robot_result}")
            settlement = terminal.get("settlement") or {}
            tx_hash = settlement.get("transaction") or settlement.get("txHash")
            if not tx_hash:
                raise RuntimeError(f"successful paid X30 action lacks transaction receipt: {terminal}")
            print("Correlated robot/tunnel/result:", json.dumps(robot_result, indent=2), flush=True)
            print("Correlated terminal status:", json.dumps(terminal, indent=2), flush=True)
            ...
            evidence = {
                ...
                "robot_result_envelope": robot_result,
                "course": {
                    **course_spec(),
