...
if (-not $AllowDirtyWorktree -and $dirtyPaths.Count -gt 0) {
    throw 'The worktree is dirty. Commit the exact runner/course first, or use -AllowDirtyWorktree only for a non-final rehearsal.'
}
...
$env:X30_MUJOCO_VIEWER_HOLD_SECONDS = [string]$HoldSeconds
...
public static class RoboPayX30WindowLayout {
...
"@
    ...
    if ($terminalHandle -ne [IntPtr]::Zero) {
        [void][RoboPayX30WindowLayout]::MoveWindow(
...
using System;
using System.Runtime.InteropServices;
public static class RoboPayX30ViewerLayout {
    [DllImport("user32.dll", SetLastError = true)]
    public static extern bool MoveWindow(IntPtr hWnd, int x, int y, int width, int height, bool repaint);
}
"@
        ...
        while ([DateTime]::UtcNow -lt $deadline) {
            ...
            foreach ($viewer in $viewers) {
                [void][RoboPayX30ViewerLayout]::MoveWindow(
...
Write-Host 'OBS sequence: bridge ready -> discovery -> unpaid 402 -> first paid 202 -> X30 Pro MuJoCo motion -> settlement -> BaseScan'
...
if ($AllowDirtyWorktree) {
    Write-Warning 'Rehearsal mode: the worktree is dirty, so this run is not acceptable as final current-commit evidence.'
}
Write-Host ''
