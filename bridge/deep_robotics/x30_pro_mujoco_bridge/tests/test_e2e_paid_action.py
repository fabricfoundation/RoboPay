"""First-action X30 proof through real Tunnel, Zenoh, MuJoCo and x402.
...
WebSocket reader, Zenoh transport, X30 bridge, and vendor-MJCF MuJoCo episode
...
from x30_pro_mujoco_bridge.model import resolve_model_dir
...
ZENOH_PORT = int(os.environ.get("X30_POSITIVE_E2E_ZENOH_PORT", "7459"))


class X30PositivePaidActionTests(unittest.TestCase):
    def test_first_paid_action_runs_vendor_mujoco_and_settles_once(self) -> None:
        ...
        try:
            ...
            proxy.start()
            with tempfile.TemporaryDirectory(prefix="x30_positive_e2e_") as temp_dir:
                temp = Path(temp_dir)
                ...
                action_id = f"x30-paid-{uuid.uuid4().hex}"
                ...
                self.assertTrue(result["task_goal_reached"])
                ...
                print("[X30 E2E] first paid action -> vendor MuJoCo inspection lane, correlated success, one settlement")
        finally:
