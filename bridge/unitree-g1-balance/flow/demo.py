...
# (skill_id, params) -- the genuine outcomes of the paid flow:
#   recover (gentle push, settles) / hold (stop, settles) / fall (hard push,
#   genuine physics failure, NO settlement).
DEMO_SCENES = [
    ("balance_recover", {}),
    ("stop", {}),
    ("balance_recover", {"push": 8.0}),   # exceeds torque authority -> falls
]
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--skill", default="balance_recover",
                    choices=[s for s, _ in DEMO_SCENES[:2]])
    ...
    ap.add_argument("--verbose", action="store_true",
                    help="verbose trace for every scene (used to render evidence)")
    ...
    if args.all:
        ...
        for skill_id, params in DEMO_SCENES:
            ...
            print("-" * 68)
            res = run_once(relay, executor, skill_id, params, verbose=args.verbose)
            m = res.get("metrics") or {}
            print(f" status={res.get('status')}  msg={res.get('message')}  "
                  f"settled={res.get('settled')}")
            if skill_id == "balance_recover":
                print(f" pitch={float(m.get('pitchRad', 0.0)):.3f} rad  "
                      f"maxPitch={float(m.get('maxPitchRad', 0.0)):.3f} rad  "
                      f"fell={m.get('fell')}  recovered={m.get('recovered')}  "
                      f"pushImpulse={m.get('pushImpulse')}  "
                      f"steps={m.get('stepsUsed')}/{m.get('stepBudget')}")
            else:
                print(f" steps={m.get('stepsUsed')}/{m.get('stepBudget')}")
        ...
        print("=" * 78)
        # success scenes settle; the hard-push fall must NOT settle
        ok = (rows[0][3] is True and rows[1][3] is True
              and rows[2][3] is False)
        print(" PASS: every success settles, the genuine fall does not."
