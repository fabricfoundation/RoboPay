...
class TestX402ChallengeFromProfiles(unittest.TestCase):
    ...
    def test_challenge_matches_payment_policy(self):
        ch = X402Challenge("balance_recover")
...
class TestRealMuJoCoCorrelated(unittest.TestCase):
    ...
    def test_real_mujoco_recover_succeeds(self):
        ex = MuJoCoExecutor()
        res = ex.execute("balance_recover", {})
        self.assertTrue(res.success, msg=f"mujoco sim failed: {res.message}")
        self.assertTrue(res.metrics.get("recovered"))
        self.assertLess(res.metrics.get("maxPitchRad", 9), g1_spec.FALL_PITCH)
        self.assertTrue(res.metrics.get("reached"))

    def test_real_mujoco_stop_holds(self):
        ex = MuJoCoExecutor()
        res = ex.execute("stop", {})
        self.assertTrue(res.success, msg=f"mujoco sim failed: {res.message}")
        self.assertLess(abs(res.metrics.get("pitchRad", 1.0)), 0.02)

    def test_real_mujoco_fall_does_not_settle(self):
        from flow.relay import Relay
        r = Relay(MuJoCoExecutor())
        resp = r.handle({
            "skill": "balance_recover", "robotId": "unitree-g1",
            "idempotencyKey": "k-mujoco-fall",
            "payment": valid_receipt(),
            "params": {"push": g1_spec.PUSH_W_FALL},
        })
        self.assertEqual(resp["status"], "failed")
        self.assertFalse(resp["settled"], "real sim fall must never settle")
