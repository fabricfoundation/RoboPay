...
SKILL_CATALOG = (
    ...
    / "laok.unitree-g1-arm-001.balance-recover.v1/skill-catalog.json"
)
...
PRICE = "0.10"
ALLOWED_ACTIONS = "balance_recover,stop"
...
class SimulatorSide:
    ...
    def _on_action(self, sample) -> None:
        ...
        res = self.executor.execute(skill_id or "balance_recover", params)
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="unitree_g1_payment_gate_") as temp_dir:
                ...
                unpaid_status, unpaid_headers, _ = http_post(
                    action_url, {"action": "balance_recover", "robot_id": ROBOT_ID}
                )
                ...
                malformed_status, _, _ = http_post(
                    action_url,
                    {"action": "balance_recover", "params": "not-an-object"},
                )
    ...
    def test_failed_execution_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="unitree_g1_fail_") as temp_dir:
                ...
                paid_status, _, _ = self._paid_post(
                    action_url, unpaid_headers, failed_id, {"push": 8.0}
                )
