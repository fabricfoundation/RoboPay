...
class TestFailureNoSettle(unittest.TestCase):
    """An execution that fails (here: a genuinely toppled robot) settles ZERO."""

    def _relay(self):
        # MuJoCo backend is a hard dependency; a push the torque-limited PD
        # cannot catch is a real physics fall (not a scripted failure).
        try:
            from flow.executor import MuJoCoExecutor
            return Relay(MuJoCoExecutor())
        except Exception:                                 # pragma: no cover
            return Relay(MockExecutor(fail_skill="balance_recover"))
