"""Protocol-accurate local Fabric/x402 harness for Atlas integration tests.

The proxy and facilitator are recording test doubles.  The Tunnel binary,
x402 middleware, Zenoh transport, and Atlas bridge remain the production code.
"""
...
def http_get(url: str):
    try:
        with urllib.request.urlopen(urllib.request.Request(url, method="GET"), timeout=35) as response:
            return response.status, dict(response.headers), response.read()
    except urllib.error.HTTPError as error:
        return error.code, dict(error.headers), error.read()
...
def poll_action_status(url: str, terminal_states: set[str], timeout: float = 30) -> dict:
    deadline = time.monotonic() + timeout
    last = None
    while time.monotonic() < deadline:
