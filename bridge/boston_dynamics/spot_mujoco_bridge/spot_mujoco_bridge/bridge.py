"""Zenoh bridge for paid Spot obstacle-course actions."""
...
from .runner import run_obstacle_course


LOGGER = logging.getLogger("robopay.spot")
ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/boston_dynamics_spot/metrics"
ROBOT_ID = "spot-mujoco-sim-01"
...
PROFILE_ID = "boston-dynamics.spot.mujoco-webots-obstacle-course.v1"
...
def _navigation_params(params: dict) -> tuple[float, str, float]:
    ...
    if isinstance(raw_speed, bool) or not isinstance(raw_speed, (int, float)):
        raise ActionContractError("INVALID_SPEED", "speedScale must be a number from 0.25 to 1.0.")
    speed_scale = float(raw_speed)
    if not math.isfinite(speed_scale) or not 0.25 <= speed_scale <= 1.0:
        raise ActionContractError("INVALID_SPEED", "speedScale must be between 0.25 and 1.0.")
...
def _visual_payment_demo() -> tuple[bool, float | None]:
    ...
    enabled = os.environ.get("SPOT_MUJOCO_VIEWER", "").strip().lower() in {"1", "true", "yes"}
    if not enabled:
        return False, None
    raw_hold = os.environ.get("SPOT_MUJOCO_VIEWER_HOLD_SECONDS", "5")
    try:
        hold_seconds = float(raw_hold)
    except ValueError as error:
        raise RuntimeError("SPOT_MUJOCO_VIEWER_HOLD_SECONDS must be a non-negative number.") from error
    if hold_seconds < 0:
        raise RuntimeError("SPOT_MUJOCO_VIEWER_HOLD_SECONDS must be a non-negative number.")
...
class SpotZenohBridge:
    ...
    def __init__(self, model_dir: str | None = None, settings: BridgeSettings | None = None):
        try:
            ...
            raise RuntimeError("Install eclipse-zenoh to run the Spot bridge.") from error
        self._zenoh = zenoh
    ...
    def _execute_navigation(
        ...
        try:
            ...
            LOGGER.exception("Spot simulator execution failed")
            result = {
    ...
    def _on_action(self, sample) -> None:  # exercised by direct and live-Zenoh tests
        ...
        with self._worker_lock:
            ...
            self._worker = threading.Thread(
                ...
                daemon=True,
                name=f"spot-action-{event.action_id}",
            )
            self._worker.start()

    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "Spot bridge %s listening on %s and publishing results on %s",
            self.robot_id,
            self.action_topic,
            self.result_topic,
        )
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the Spot bridge as a standalone Zenoh worker."""

    logging.basicConfig(level=logging.INFO)
    SpotZenohBridge().spin()
