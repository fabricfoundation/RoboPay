...
def run_sim2sim_validation(timeout_seconds: int = 150) -> dict:
    ...
    mujoco_result = run_obstacle_course()
    ...
    return {
        "task": "navigate_obstacle_course_sim2sim",
        ...
        "note": (
            "Joint-zero and yaw-torque adapters differ because the Menagerie MJCF and "
            "Cyberbotics PROTO use different joint conventions; the policy inputs, route, "
            "timing, gait phase, and steering command are shared."
        ),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the paired Spot sim-to-sim validation.")
