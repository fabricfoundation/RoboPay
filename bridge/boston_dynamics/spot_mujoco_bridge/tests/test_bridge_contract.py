...
from spot_mujoco_bridge.bridge import SpotZenohBridge, _load_event_parser
...
def _bridge() -> SpotZenohBridge:
    bridge = SpotZenohBridge.__new__(SpotZenohBridge)
    bridge.robot_id = "spot-contract-test"
...
class SpotBridgeContractTests(unittest.TestCase):
    def test_parser_accepts_profile_skill_id_and_preserves_metadata(self):
        parser = _load_event_parser()
        parsed = parser(json.dumps(_event("navigate_obstacle_course")).encode("utf-8"))
        self.assertIsNotNone(parsed)
        self.assertEqual(parsed.action, "navigate_obstacle_course")
    ...
    def test_parser_rejects_missing_skill_and_invalid_params(self):
        ...
        self.assertIsNone(
            parser(b'{"payload":{"action":"navigate_obstacle_course","params":[]}}')
        )
        missing_correlation = _event("navigate_obstacle_course")
        del missing_correlation["idempotency_key"]
        self.assertIsNone(parser(json.dumps(missing_correlation).encode("utf-8")))

        tampered = _event("navigate_obstacle_course", {"side": "left"})
        ...
        for simulator_result, expected_status in (
            ...
            with self.subTest(expected_status=expected_status):
                ...
                with patch.dict(os.environ, {}, clear=True), patch(
                    "spot_mujoco_bridge.bridge.run_obstacle_course",
                    return_value=simulator_result,
                ):
                    bridge._on_action(_Sample(_event("navigate_obstacle_course")))
                    bridge._worker.join(timeout=2)
                result = bridge._result_publisher.documents[-1]
                self.assertEqual(result["action_id"], "action-1")
                self.assertEqual(result["robot_id"], "spot-contract-test")
                self.assertEqual(result["skill_id"], "navigate_obstacle_course")
    ...
    def test_unknown_and_invalid_actions_fail_without_simulation(self):
        cases = (
            ...
            (_event("navigate_obstacle_course", {"maxDurationSec": 61}), "INVALID_DURATION"),
            (_event("navigate_obstacle_course", {"speedScale": 1.5}), "INVALID_SPEED"),
            (_event("navigate_obstacle_course", {"side": "right"}), "INVALID_SIDE"),
            (_event("navigate_obstacle_course", {"request_id": "legacy"}), "INVALID_PARAMS"),
            (_event("stop", {"unexpected": True}), "INVALID_PARAMS"),
        )
        for document, expected_code in cases:
            with self.subTest(expected_code=expected_code):
                bridge = _bridge()
                with patch("spot_mujoco_bridge.bridge.run_obstacle_course") as runner:
    ...
    def test_wrong_robot_is_dropped_without_simulator_or_foreign_result(self):
        bridge = _bridge()
        document = _event("navigate_obstacle_course")
    ...
    def test_stop_interrupts_navigation_and_publishes_both_terminal_results(self):
        ...
        with patch.dict(os.environ, {}, clear=True), patch(
            ...
            bridge._on_action(_Sample(_event("navigate_obstacle_course", action_id="navigate-1")))
