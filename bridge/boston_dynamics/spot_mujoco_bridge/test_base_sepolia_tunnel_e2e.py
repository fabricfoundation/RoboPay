"""Live Base Sepolia x402 -> Tunnel -> Spot MuJoCo settlement test.
...
def main() -> int:
    ...
    robot_id = os.environ.get("ROBOT_ID", f"spot-mujoco-base-sepolia-{int(time.time())}")
    request_id = f"spot-obstacle-course-{int(time.time())}"
    ...
    with tempfile.TemporaryDirectory(prefix="robopay_spot_base_sepolia_") as temp_dir:
        temp = Path(temp_dir)
        ...
        bridge = subprocess.Popen(
            [sys.executable, "-m", "spot_mujoco_bridge.bridge"],
            cwd=PACKAGE_ROOT,
            env=bridge_env,
        ...
        try:
            ...
            if discovered_skills != {"navigate_obstacle_course", "stop"}:
            ...
            print("Robot discovery: Spot simulator-only")
            print("Skill discovery: navigate_obstacle_course, stop @ 0.001 USDC")
