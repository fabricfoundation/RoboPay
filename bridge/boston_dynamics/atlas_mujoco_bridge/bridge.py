"""Zenoh bridge for paid Atlas obstacle-nav actions."""
...
class AtlasZenohBridge:
    ...
    def _execute_navigation(self, event, max_duration, side, speed_scale) -> None:
        try:
            result = run_obstacle_nav(
                self._model_dir, max_duration, side,
                speed_scale=speed_scale,
                stop_requested=self._stop_event.is_set,
            )
        except Exception as error:
            LOGGER.exception("Atlas simulator execution failed")
            result = {"error_code": "SIMULATOR_EXECUTION_ERROR", "message": str(error), "success": False}
    ...
    def _on_action(self, sample) -> None:
        ...
        if action == "stop":
            ...
            self._publish(event, "success" if stop_confirmed else "failure", {
                "message": "Safe stop applied" if stop_confirmed else "Safe stop was not confirmed within 5 seconds",
        ...
        with self._worker_lock:
            ...
            self._worker = threading.Thread(
                ...
                args=(event, max_duration, side, speed_scale),
                daemon=True,
