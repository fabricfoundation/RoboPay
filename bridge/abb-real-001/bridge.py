"""Zenoh bridge for paid abb-real-001 ``pick_object`` actions.
...
LOGGER = logging.getLogger("robopay.abb")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/abb-real-001/metrics"

ROBOT_ID = "abb-real-001"
ALLOWED_ACTIONS = {'"mobile_pick"'}
PROFILE_ID = "abb.abb-real-001.mujoco-sim.v1"
...
class FabricZenohBridge:
    ...
    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "abb-real-001 bridge %s listening on %s; publishing results on %s",
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the abb-real-001 bridge as a standalone Zenoh worker."""
