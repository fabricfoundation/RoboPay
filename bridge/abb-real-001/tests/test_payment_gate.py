"""Exercise abb-real-001's x402 payment gate through the real Go Tunnel binary.
...
ROBOT_ID = "abb_arm_001_payment_gate"
...
class FabricPaymentGateTests(unittest.TestCase):
    def test_unpaid_and_invalid_payment_fail_closed(self) -> None:
        ...
        try:
            proxy.start()
            with tempfile.TemporaryDirectory(prefix="abb_payment_gate_") as temp_dir:
                temp = Path(temp_dir)
                ...
                # Prove the 402 challenge is wired for abb-real-001.
                required = json.loads(
                ...
                tampered_id = f"abb-tampered-payment-{uuid.uuid4().hex}"
