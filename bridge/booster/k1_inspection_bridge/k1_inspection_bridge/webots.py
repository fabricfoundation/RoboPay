"""Launch the native Webots validation for the supported K1 station."""
...
SCENE = PACKAGE_ROOT / "scenes" / "k1_inspection_station.wbt"
RESULT = PACKAGE_ROOT / "scenes" / "webots_inspection_result.json"
PROTO = PACKAGE_ROOT / "scenes" / "K1.proto"
...
def run_webots_validation(timeout_seconds: int = 120) -> dict:
    ...
    environment.pop("BOOSTER_K1_WEBOTS_HOLD_VIEWER", None)
    process = subprocess.Popen(
        [str(executable), "--batch", "--mode=fast", "--no-rendering", "--stdout", "--stderr", str(SCENE)],
        cwd=SCENE.parent,
        env=environment,
...
def launch_webots_viewer() -> int:
    ...
    environment["WEBOTS_CONTROLLER_PATH"] = str(PACKAGE_ROOT / "controllers")
    environment["PYTHONPATH"] = str(PACKAGE_ROOT) + os.pathsep + environment.get("PYTHONPATH", "")
    environment["BOOSTER_K1_WEBOTS_HOLD_VIEWER"] = "1"
    environment.setdefault("WEBOTS_HOME", str(_webots_home(executable)))
    process = subprocess.Popen([str(executable), "--mode=realtime", str(SCENE)], cwd=SCENE.parent, env=environment)
    return process.pid
