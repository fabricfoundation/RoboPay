"""Fail-closed Zenoh execution bridge for the paid Booster K1 skill."""
...
LOGGER = logging.getLogger("robopay.booster_k1")
ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/booster_k1/metrics"
...
def _visual_payment_demo() -> tuple[bool, float | None, float, float]:
    ...
    try:
        ...
        start_hold = float(os.environ.get("BOOSTER_K1_VIEWER_START_HOLD_SECONDS", "0"))
...
class K1ZenohBridge:
    def __init__(self, model_dir: str | None = None, settings: BridgeSettings | None = None):
        ...
        ready_path = os.environ.get("BOOSTER_K1_READY_FILE", "").strip()
        if ready_path:
    ...
    def _execute(self, event, duration: float, targets: tuple[str, ...], speed: float, viewer: bool, hold: float | None, target_hold: float, start_hold: float) -> None:
        ...
        LOGGER.info(
            "K1 action completed action_id=%s status=%s targets_confirmed=%s",
            event.action_id,
    ...
    def _on_action(self, sample) -> None:
        ...
        with self._lock:
            ...
            LOGGER.info(
                "Authorized K1 ActionEvent accepted action_id=%s robot_id=%s targets=%s",
                event.action_id,
                event.robot_id,
                targets,
            )
            self._worker = threading.Thread(target=self._execute, args=(event, duration, targets, speed, viewer, hold, target_hold, start_hold), daemon=True, name=f"k1-action-{event.action_id}")
            self._worker.start()

    def spin(self) -> None:
        LOGGER.info(
            "K1 bridge robot_id=%s listening=%s results=%s metrics=%s",
            self.robot_id,
