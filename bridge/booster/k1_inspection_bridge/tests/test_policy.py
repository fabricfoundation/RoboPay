...
class K1PolicyTests(unittest.TestCase):
    ...
    def test_feedback_is_required_before_target_advances(self):
        ...
        policy.reset()
        plan = policy.compute_plan({"sim_time": 10.0, "upper_joint_positions": [0.0] * 10})
    ...
    def test_measured_tolerance_and_dwell_complete_target(self):
        ...
        policy.compute_plan({"sim_time": 1.0, "upper_joint_positions": measured})
        policy.compute_plan({"sim_time": 1.41, "upper_joint_positions": measured})
    ...
    def test_official_mjcf_completes_all_targets(self):
        ...
        self.assertEqual(result["model_source_commit"], "508cbee6ca9ae6fbc8c0b38dd58785a6f3fc61a2")
        self.assertEqual(result["support_fixture"], "fixed-base safety stand")
