...
ROBOT_ID = "booster_k1_no_settlement"
PORT = int(os.environ.get("BOOSTER_K1_NO_SETTLE_ZENOH_PORT", "7468"))
...
class InjectedSimulator:
    ...
    def on_action(self, sample):
        ...
        self.publisher.put(json.dumps({
            ...
            "status": "failure", "result": {"error_code": "INJECTED_K1_FAILURE"},
        }).encode())
...
class K1NoSettlementTests(unittest.TestCase):
    def test_failure_timeout_and_replay_never_settle(self):
        ...
        try:
            proxy.start()
            with tempfile.TemporaryDirectory(prefix="k1_nosettle_") as temporary:
                ...
                failed_id = f"k1-failure-{uuid.uuid4().hex}"
                ...
                simulator.mode = "silent"; timeout_id = f"k1-timeout-{uuid.uuid4().hex}"; timeout_signature = payment_signature_from_402(headers)
                ...
                payment_replay_id = f"k1-payment-replay-{uuid.uuid4().hex}"
