"""Policy-driven obstacle navigation for Booster K1 humanoid.
...
K1 is a humanoid robot with the following constraints:
...
class ObstacleNavSkill(RobotSkill):
    ...
    def __init__(
        ...
        self._max_speed = min(max_speed, 0.8)  # K1 hard limit
        self._max_angular = min(max_angular, 0.3)  # K1 hard limit
    ...
    def step(self, state: Dict[str, Any]) -> SkillResult:
        ...
        # K1 is a humanoid: forward-only locomotion (vx >= 0)
        ...
        # K1 cannot walk backward — clamp to non-negative
        speed = max(0.0, speed)
