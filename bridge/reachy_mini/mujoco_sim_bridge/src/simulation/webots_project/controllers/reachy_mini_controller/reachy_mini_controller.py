...
Runs the FSM tracking policy inside the Webots physics engine for each target
and writes aggregated metrics to a JSON file.
"""
import sys
import os
import json
...
MAX_PER_TARGET = 3.5   # 3.5s per object = ~10.5s total video demo
...
def main():
    ...
    for target_name in TARGETS:
        ...
        print(f"[WebotsController] Completed {target_name.upper()} — success_rate={s['tracking_success_rate']:.2f}")
    ...
    print(f"[WebotsController] ALL TARGETS COMPLETED! avg_score={avg_score:.3f}")
