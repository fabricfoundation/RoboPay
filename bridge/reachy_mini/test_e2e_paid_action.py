...
class _ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if clean_path == "/ws":
            ...
            return
        # Forward status polls (GET /robots/<id>/action/<action_id>/status)
        # to the Tunnel over the same WebSocket envelope used for actions.
    ...
    def _forward_to_tunnel(self, method, path, body):
...
def _poll_action_status(status_url, terminal_states, timeout=90):
    """Poll GET /action/<id>/status until a terminal state is reached.

    Returns the decoded terminal status body; raises AssertionError when the
    deadline passes while still pending/unknown.
    """
    deadline = time.monotonic() + timeout
    last = None
    while time.monotonic() < deadline:
        status, _, body = _http_get(status_url)
        if status == 200:
    ...
    raise AssertionError(
        f"status endpoint never reached {terminal_states}; last observation: {last}"
    )
    ...
    payment = {
        ...
        "payload": {
            ...
            "authorization": {
                ...
                # A fresh nonce per payment: the Tunnel's durable idempotency
                # store rejects a byte-identical PAYMENT-SIGNATURE replay.
...
class TestEndToEndPaidAction(unittest.TestCase):
    ...
    def test_positive_paid_request_drives_simulator(self):
        ...
        try:
            ...
            # Immediate accepted/pending contract: 202 with the actionId now,
            # terminal result + settlement receipt via the status endpoint.
            self.assertEqual(paid_status, 202)
            self.assertEqual(response_body.get("status"), "accepted")
            self.assertEqual(response_body.get("state"), "pending")
            self.assertEqual(response_body.get("action_id"), request_id)
            self.assertEqual(
            ...
            print(f"[E2E] terminal status={terminal}")
            self.assertEqual(terminal["state"], "succeeded")
            self.assertEqual(terminal["action_id"], request_id)
            self.assertTrue(terminal["settled"], "success must settle the payment")
            self.assertTrue(
                terminal.get("settlement", {}).get("transaction"),
                "settlement receipt must carry the transaction",
            )
