"""Zenoh bridge for paid kuka-real-001 ``pick_object`` actions.
...
LOGGER = logging.getLogger("robopay.kuka")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/kuka-real-001/metrics"

ROBOT_ID = "kuka-real-001"
ALLOWED_ACTIONS = {'"sort_and_route"'}
PROFILE_ID = "kuka.kuka-real-001.mujoco-sim.v1"
...
class FabricZenohBridge:
    ...
    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "kuka-real-001 bridge %s listening on %s; publishing results on %s",
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the kuka-real-001 bridge as a standalone Zenoh worker."""
