...
class TestRealMuJoCoCorrelated(unittest.TestCase):
    ...
    def test_real_mujoco_door_opens(self):
        ex = MuJoCoExecutor()
        res = ex.execute("open_door", {"door": "open"})
        self.assertTrue(res.success, msg=f"mujoco sim failed: {res.message}")
        self.assertGreater(res.metrics.get("doorAngle", 0), 0.5)
        self.assertEqual(res.metrics.get("handleState"), "gripped")
