#!/bin/sh
# Fetch the model assets used by the Spot simulators:
#   - MuJoCo model:  google-deepmind/mujoco_menagerie boston_dynamics_spot
#                    (BSD-3-Clause; requires MuJoCo >= 3.1.3)
# The PyBullet half of the sim-to-sim check uses the committed kinematic
# URDF simulation/pybullet/spot_simple_kin.urdf (generated once from the
# rai-opensource/spot_description `spot_simple.urdf.xacro`, meshes stripped
# for pure kinematics; see simulation/README.md).
set -e
cd "$(dirname "$0")"
...
echo "OK: models ready (menagerie boston_dynamics_spot)"
