"""
Webots controller for AGIBot X2 obstacle navigation.

Runs the SAME policy as MuJoCo for Sim-to-Sim validation.
Reports metrics from Webots physics engine.

AGIBot X2 velocity limits:
    vx: [0, 1.0] m/s
    wz: [-0.3, 0.3] rad/s

Usage:
    Place in simulation/webots/controllers/x2_navigation/
"""
import json
import math
import sys
...
class X2VelocityPolicy:
    ...
    def compute_action(self, pos, heading):
        ...
        vx = base_speed * max(0, math.cos(angle_error))
        wz = 0.5 * angle_error

        # Clamp to AGIBot X2 limits
        vx = max(self.VX_MIN, min(self.VX_MAX, vx))
        wz = max(self.WZ_MIN, min(self.WZ_MAX, wz))
...
def run_webots_simulation():
    """Run obstacle navigation in Webots for AGIBot X2."""
    ...
    # Policy — same parameters as MuJoCo
    obstacles = [(2.5, 0.0), (5.0, 1.5), (7.0, -1.0)]
    policy = X2VelocityPolicy(goal=(10.0, 0.0), obstacles=obstacles)
    ...
    print("AGIBot X2 Webots simulation started")
    print(f"Goal: {policy.goal}, Obstacles: {obstacles}")
    print(f"Velocity limits: vx [0, 1.0], wz [-0.3, 0.3]")
    print("=" * 60)
    ...
    while robot.step(timestep) != -1:
        ...
        # Policy computes action (vx, wz)
        vx, wz = policy.compute_action((pos[0], pos[1]), heading)
