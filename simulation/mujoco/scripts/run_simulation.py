"""
MuJoCo simulation runner for LimX TRON1 obstacle navigation.

This script:
1. Loads the TRON1 MuJoCo scene with obstacles
...
class PotentialFieldPolicy:
    """Policy-driven obstacle navigation using potential fields for LimX TRON1.
...
def run_simulation(
    ...
    """Run MuJoCo simulation with policy-driven navigation for TRON1.
    ...
    steps_per_control = int(0.01 / dt)
    max_steps = int(duration / dt)

    print(f"Starting TRON1 simulation: {duration}s, {max_steps} steps")
    print(f"Goal: {goal_pos}, Obstacles: {obstacles}")
    print(f"Velocity limits: vx [0, 0.8], wz [-0.3, 0.3]")
    print("=" * 60)
    ...
    for step in range(max_steps):
        ...
        if step % steps_per_control == 0:
            ...
            data.ctrl[0] = ctrl_x * 80
            data.ctrl[1] = ctrl_y * 30
            data.ctrl[2] = ctrl_yaw * 30
    ...
    print("\n" + "=" * 60)
    print("TRON1 SIMULATION COMPLETE")
    print("=" * 60)
    print(json.dumps(metrics.to_dict(), indent=2))
...
def main():
    parser = argparse.ArgumentParser(description="MuJoCo LimX TRON1 obstacle navigation simulation")
    parser.add_argument("--scene", default="scenes/tron1_obstacle_nav.xml", help="Scene XML file")
