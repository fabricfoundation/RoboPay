...
A paid RoboPay action arriving on the tunnel's Zenoh topic starts a Go2
...
The same joint configurations are recomputed in PyBullet by loading the
*same* `go2.xml` through `pybullet.loadMJCF`, with a measured agreement
...
(|bodyZ - 0.283| < 0.02, where `home` is the robot's own settled resting
...
```sh
cd simulation
./setup.sh   # fetch the official Go2 model assets (pinned commit)
```
...
The skill catalog lives in `go2/skills.json` (7 priced skills, $0.002 each;
...
Payment gate: `go2/payment_gate.py` reimplements the tunnel's x402
...
- `ROBOPAY_ACTION_TOPIC` (`robot/tunnel/action`), `ROBOPAY_RESULT_TOPIC`
  ...
  `tunnel/config.json`)
- `GO2_MODEL_PATH` (default `models/mujoco_menagerie/unitree_go2/scene.xml`
...
`test_sim2sim_go2.py` captures each skill's salient pose in MuJoCo (wave peak
lift, sit deepest crouch, bow max pitch, nod max dip, end of turn, home) and
recomputes the exact same joint configuration in PyBullet by loading the
*same* `go2.xml` via `pybullet.loadMJCF` — no hand-rolled URDF in between.
Foot-sphere centres agree to well under 1 cm across all poses and all four
feet (`pybullet/sim2sim_report.json`) — the two engines run the same
...
{"actionId": "act_…", "skill": "wave", "status": "success",
 "result": {"message": "Action completed",
            "metrics": {"pawLift": 0.19, "bodyZ": 0.33, "…": "…"}}}
...
- **MuJoCo fails to load the model**: the menagerie Go2 needs MuJoCo 3.1.3+.
...
├── go2/
│   ├── go2_control.py       joint-space skill controller on MuJoCo
...
│   └── (dev-only calibration scripts)
└── pybullet/
    ├── test_sim2sim_go2.py  MuJoCo poses recomputed in PyBullet, compared
    └── sim2sim_report.json
```
