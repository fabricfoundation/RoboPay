#!/usr/bin/env python3
"""
Sim-to-Sim Validation for LimX TRON1 Obstacle Navigation.
...
def run_mujoco_validation(duration: float = 60.0) -> Optional[SimResult]:
    ...
    scene_path = Path(__file__).parent.parent / "mujoco" / "scenes" / "tron1_obstacle_nav.xml"
...
def run_webots_validation(duration: float = 60.0) -> Optional[SimResult]:
    ...
    world_path = Path(__file__).parent.parent / "webots" / "worlds" / "tron1_obstacle_nav.wbt"
...
def compare_results(
    ...
    comparison = {
        "robot": "tron1",
        "velocity_limits": {"vx": [0, 0.8], "wz": [-0.3, 0.3]},
        "mujoco": {
...
def main():
    parser = argparse.ArgumentParser(description="Sim-to-Sim validation for LimX TRON1 obstacle navigation")
    ...
    print("=" * 60)
    print("SIM-TO-SIM VALIDATION: LimX TRON1 Obstacle Navigation")
    print("Velocity limits: vx [0, 0.8], wz [-0.3, 0.3]")
    print("=" * 60)
