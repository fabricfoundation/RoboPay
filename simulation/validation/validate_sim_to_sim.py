...
def run_mujoco_validation(duration: float = 60.0) -> Optional[SimResult]:
    ...
    scene_path = Path(__file__).parent.parent / "mujoco" / "scenes" / "limx-tron1_obstacle_nav.xml"
...
def run_webots_validation(duration: float = 60.0) -> Optional[SimResult]:
    ...
    world_path = Path(__file__).parent.parent / "webots" / "worlds" / "limx-tron1_obstacle_nav.wbt"
...
def main():
    parser = argparse.ArgumentParser(description="Sim-to-Sim validation for TRON1 obstacle navigation")
