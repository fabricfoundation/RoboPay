"""Zenoh bridge for paid xarm-real-001 ``pick_object`` actions.
...
LOGGER = logging.getLogger("robopay.xarm")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/xarm-real-001/metrics"

ROBOT_ID = "xarm-real-001"
ALLOWED_ACTIONS = {'"push_object"'}
PROFILE_ID = "xarm.xarm-real-001.mujoco-sim.v1"
...
class FabricZenohBridge:
    ...
    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "xarm-real-001 bridge %s listening on %s; publishing results on %s",
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the xarm-real-001 bridge as a standalone Zenoh worker."""
